...
Процедура РеализацияМедикаментовСоздать(Шапка, Детали)
	...
	Если СтрСравнить(Шапка.BILLTYP, "S1") = 0 Тогда
		Корректируемый	= ДокументРеализацияНайти(Новый Структура("BILLDOC,BILLDATE", Шапка.REVREC, Шапка.IADE_REF_TARIH));
...
Процедура ВозвратНоменклатурыПродажСоздать(Шапка, Детали)
	...
	Если СтрСравнить(Шапка.BILLTYP, "S1") = 0 Тогда
		...
		Корректируемый	= ДокументВозвратНоменклатурыПродажНайти(Новый Структура("BILLDOC,BILLDATE", Шапка.REVREC, Шапка.IADE_REF_TARIH));
		ДокументПометить(Корректируемый);
	КонецЕсли;
