...
Процедура Логировать(Слова)
	Если ЗначениеЗаполнено(ExchangeHost()) Тогда
		rMan = РегистрыСведений.IntegrationLogs.СоздатьМенеджерЗаписи();
		rMan.ExchangeHost = ExchangeHost;
		rMan.Период = ТекущаяДата();
		rMan.Log = Слова;
		Попытка
			rMan.Записать();
		Исключение
			ЗаписьЖурналаРегистрации("ОбменСАП", УровеньЖурналаРегистрации.Ошибка, РегламентныеЗадания.НайтиПредопределенное("ОбменСАП"), ОписаниеОшибки(), Слова);
		КонецПопытки;
	Иначе
		ЗаписьЖурналаРегистрации("ОбменСАП", УровеньЖурналаРегистрации.Ошибка, РегламентныеЗадания.НайтиПредопределенное("ОбменСАП"),, Слова);
	КонецЕсли;
КонецПроцедуры
...
Функция Настроить(Объект) Экспорт
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Или НЕ ЗначениеЗаполнено(Объект.Ответственный) Тогда
		...
		Объект.ExchangeHost	= ExchangeHost();
	КонецЕсли;
	Возврат ЗначениеЗаполнено(Объект.Организация);
КонецФункции

Функция ExchangeHost()
	Если НЕ ЗначениеЗаполнено(ExchangeHost) Тогда
		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	Полный.Код = &Код");
		Запрос.УстановитьПараметр("Код",	"sap");
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			ExchangeHost = Выборка.Ссылка;
		КонецЕсли;
	КонецЕсли;
	Возврат ExchangeHost;
КонецФункции
...
Функция ОсновноеСредствоНайти(ИнвентарныйНомер)
	...
	Запрос.УстановитьПараметр("Номер",			УдалитьЛижирующие0000(ИнвентарныйНомер));
	Запрос.УстановитьПараметр("Организация",	Организация);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Ответ =  Выборка.Ссылка;
...
Функция УдалитьЛижирующие0000(Знач Слово)
...
Процедура ВыпискаБанкаСоздать(Шапка, Детали)
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		мДетали = Новый Массив;
		Для Каждого Деталь Из Детали Цикл
			Если мДетали.Найти(Деталь.DOCNO) = Неопределено Тогда
				Виктимз	= Док.Платежи.НайтиСтроки(Новый Структура("ПлатежныйДокумент", Деталь.DOCNO));
				Для Каждого ТекСтрока Из Виктимз Цикл
					Док.Платежи.Удалить(ТекСтрока);
				КонецЦикла;
				мДетали.Добавить(Деталь.DOCNO);
			КонецЕсли;
		КонецЦикла;
