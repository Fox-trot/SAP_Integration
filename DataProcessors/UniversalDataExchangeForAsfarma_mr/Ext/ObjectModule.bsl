...
Перем УзелОбменаЗагрузкаДанных;
...
//
// Примеры:
//  РазложитьСтрокуВМассивПодстрок_(",один,,два,", ",") - возвратит массив из 5 элементов, три из которых  - пустые строки;
//  РазложитьСтрокуВМассивПодстрок_(",один,,два,", ",", Истина) - возвратит массив из двух элементов;
//  РазложитьСтрокуВМассивПодстрок_(" один   два  ", " ") - возвратит массив из двух элементов;
//  РазложитьСтрокуВМассивПодстрок_("") - возвратит пустой массив;
//  РазложитьСтрокуВМассивПодстрок_("",,Ложь) - возвратит массив с одним элементом "" (пустой строкой);
//  РазложитьСтрокуВМассивПодстрок_("", " ") - возвратит массив с одним элементом "" (пустой строкой);
//
Функция РазложитьСтрокуВМассивПодстрок_(Знач Строка, Знач Разделитель = ",", Знач ПропускатьПустыеСтроки = Неопределено) Экспорт
...
Процедура ИнициализироватьВедениеПротоколаОбмена() Экспорт
	...
	мФайлПротоколаДанных = Новый ЗаписьТекста(ИмяФайлаПротоколаОбмена, КодировкаТекста.UTF8, , ДописыватьДанныеВПротоколОбмена) ;
...
Процедура ИнициализироватьВедениеПротоколаОбменаДляЭкспортаОбработчиков()
	...
	мФайлПротоколаДанных = Новый ЗаписьТекста(ИмяВременногоФайлаПротоколаОбмена, КодировкаТекста.UTF8);
	...
	Пока ПравилаОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "ПоискПоТабличнымЧастям" Тогда
			...
			Для Номер = 1 По СтрЧислоСтрок(Значение) Цикл
				...
				СтрокаТаблицы.ПоляПоискаТЧ = РазложитьСтрокуВМассивПодстрок_(СтрокаПоиска);
...
Процедура ОткрытьФайлЗагрузки(ТолькоПрочитатьШапку=Ложь, ДанныеФайлаОбмена = "") Экспорт
	...
	Исключение
		...
		Возврат;
	КонецПопытки;
	//+DimaB попытка/исключение если нет файла-подтверждения
	Попытка
		ФайлОбмена.Прочитать();
	Исключение
		ФлагОшибки = Истина;
	...
	Попытка
		...
		Пока ФайлОбмена.Прочитать() Цикл
			...
			ИначеЕсли (ИмяУзла = "ФайлОбмена") И (ФайлОбмена.ТипУзла = одТипУзлаXML_КонецЭлемента) Тогда
				...
				//+ mr {21.06.2018 kobiden gelen XML üzerinde "ДанныеПоОбмену" ve "ДанныеПоФоновомуОбмену" geliyor. Onu atlamak gerekli, o bir hata değil.
				//DimaB Или (ИмяУзла = "ДанныеПоФоновомуОбмену") 
...
//  Нет.
// 
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных(РезультирующаяСтрокаСИнформациейОбОшибке="") Экспорт
	...
	ФайлОбмена.Закрыть();
	//DimaB РезультирующаяСтрокаСИнформациейОбОшибке
	Если РезультирующаяСтрокаСИнформациейОбОшибке = "" Тогда
...
//  Нет.
//
Процедура ВыполнитьЗагрузку(РезультирующаяСтрокаСИнформациейОбОшибке = "") Экспорт
	...
	КонецЕсли;	
    //DimaB РезультирующаяСтрокаСИнформациейОбОшибке
	ПроизвестиЧтениеДанных(РезультирующаяСтрокаСИнформациейОбОшибке);
	ВыполнитьДействияПослеЗавершенияЧтенияДанных(РезультирующаяСтрокаСИнформациейОбОшибке); 
...
Процедура ПередатьОдинПараметрВПриемник(Имя, ИсходноеЗначениеПараметра, ПравилоКонвертации = "") Экспорт
	...
	Если ПустаяСтрока(ПравилоКонвертации) Тогда
		...
		//+ mr {19.06.2018 Примитивные типы надо указать или на Русском или на Анг языке. А то выдается ошибка
		//УстановитьАтрибут(УзелПараметра, "Тип", Строка(ТипЗнч(ИсходноеЗначениеПараметра)));
		ИсходноеЗначение_Тип = ТипЗнч(ИсходноеЗначениеПараметра);
		Если ИсходноеЗначение_Тип = Тип("String") Тогда 
			УстановитьАтрибут(УзелПараметра, "Тип", "String");
		ИначеЕсли ИсходноеЗначение_Тип = Тип("Number") Тогда 
			УстановитьАтрибут(УзелПараметра, "Тип", "Number");
		ИначеЕсли ИсходноеЗначение_Тип = Тип("Null") Тогда 
			УстановитьАтрибут(УзелПараметра, "Тип", "Null");
		ИначеЕсли ИсходноеЗначение_Тип = Тип("Undefined") Тогда 
			УстановитьАтрибут(УзелПараметра, "Тип", "Undefined");
		ИначеЕсли ИсходноеЗначение_Тип = Тип("Date") Тогда 
			УстановитьАтрибут(УзелПараметра, "Тип", "Date");
		ИначеЕсли ИсходноеЗначение_Тип = Тип("Boolean") Тогда 
			УстановитьАтрибут(УзелПараметра, "Тип", "Boolean");
		ИначеЕсли ИсходноеЗначение_Тип = Тип("Type") Тогда 
			УстановитьАтрибут(УзелПараметра, "Тип", "Type");
		Иначе 
			УстановитьАтрибут(УзелПараметра, "Тип", Строка(ТипЗнч(ИсходноеЗначениеПараметра)));
		КонецЕсли;
		//- mr }19.06.2018 Примитивные типы надо указать или на Русском или на Анг языке. А то выдается ошибка
