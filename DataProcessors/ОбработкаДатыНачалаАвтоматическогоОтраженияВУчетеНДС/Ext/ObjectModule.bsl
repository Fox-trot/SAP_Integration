...
Процедура ОпределитьСчетаУчетаНДС(Записи_НезаполненыСчетаУчета,ТЧ_ДопСведения,Записи_ЗаменяемыеСтрокиДопСведений, РаспределяемыеКолонки)
	...
	Для каждого Строка Из БазисРаспределения Цикл
		Если ЗначениеНеЗаполнено(Строка.СчетУчетаНДС) или (Строка.СуммаБезНДС = 0 и Строка.НДС = 0) Тогда
	...
	БазисПоОтбору.Колонки.Добавить("НДС",ПолучитьОписаниеТиповЧисла(15,2));
	БазисПоОтбору.Колонки.Добавить("СуммаБезНДС",ПолучитьОписаниеТиповЧисла(15,2));
	...
	Для Счетчик = 0 по Записи_НезаполненыСчетаУчета.Количество()-1 Цикл
		...
		Иначе
			...
			Для каждого Колонка Из Распределение Цикл
				...
				МассивРезультата = РаспределитьПропорционально(Строка[Колонка.Ключ],БазисПоОтбору.ВыгрузитьКолонку(Колонка.Значение));
...
Процедура СформироватьНДСПоПриобретеннымЦенностям(ЧисловыеКолонкиВводНачОстатков)
	...
	Пока ВыборкаПоОрганизациям.Следующий() Цикл
		...
		УстановитьНомерДокумента(ДокументВводОстатков);
		...
		Пока ВыборкаПоПоставщикам.Следующий() Цикл
			...
			Пока ВыборкаПоСФ.Следующий() Цикл
				...
				Если Не ЗначениеНеЗаполнено(ВыборкаПоСФ.СчетФактура) Тогда
					Если ЕстьРеквизитДокумента("ДоговорКонтрагента", ВыборкаПоСФ.СчетФактура.Метаданные()) Тогда
				...
				Пока ВыборкаПоСобытию.Следующий() Цикл
					...
					Если ВыборкаПоСобытию.СуммаБезНДСПредъявлено0 <> 0 Тогда
						...
						СуммаБезНДСРаспределение 	= РаспределитьПропорционально(ВыборкаПоСобытию.СуммаБезНДСПредъявлено0, МассивРаспределения);
						НДСРаспределения 			= РаспределитьПропорционально(ВыборкаПоСобытию.НДСПредъявлено0, МассивРаспределения);
					...
					СтрокаРасчетовСКонтрагентами.ВалютнаяСуммаВзаиморасчетов= ПересчитатьИзВалютыВВалюту(СтрокаРасчетовСКонтрагентами.СуммаВзаиморасчетов, 
					...
					Если ЗначениеНеЗаполнено(СтрокаДоп.СчетУчетаНДС) Тогда
		...
		Пока ВыборкаПоПоставщикам.Следующий() Цикл
			...
			Пока ВыборкаПоСФ.Следующий() Цикл
				...
				Пока ВыборкаПоСобытию.Следующий() Цикл
					...
					Если ЗначениеНеЗаполнено(СтрокаСФ.Контрагент) Тогда
						Продолжить;
					КонецЕсли;
				...
				Пока ВыборкаПоВидуЦенности.Следующий() Цикл
					...
					Пока ВыборкаПоСтавке.Следующий() Цикл
						...
						Если ТипЗнч(ВыборкаПоСФ.СчетФактура) = Тип("ДокументСсылка.СчетФактураВыданный") И 
							НЕ ЗначениеНеЗаполнено(ВыборкаПоСФ.СчетФактура.ДокументОснование) Тогда
						...
						Пока ВыборкаПоСобытию.Следующий() Цикл
							...
							СтрокаДоп.ВидЦенности						= ВыборкаПоСобытию.ВидЦенности;
							СтрокаДоп.СтавкаНДС							= ВыборкаПоСобытию.СтавкаНДС;
							Если ЗначениеНеЗаполнено(СтрокаСФ.ВалютаАванса) Тогда
							...
							СтрокаРасчетовСКонтрагентами.ВалютаВзаиморасчетов 		= СтрокаСФ.ВалютаАванса;
...
Функция ОпределитьСчетУчетаИСтавкуНДСПоПартии(ВыборкаПоСФ, СтрокиПоСФ)
	...
	Результат.Колонки.Добавить("Стоимость",ПолучитьОписаниеТиповЧисла(15,2));
	Результат.Колонки.Добавить("НДС",ПолучитьОписаниеТиповЧисла(15,2));
	Результат.Колонки.Добавить("Количество",ПолучитьОписаниеТиповЧисла(15,3));
	...
	РезультатРаспределенияСтоимость = РаспределитьПропорционально(ВыборкаПоСФ.Стоимость,Результат.ВыгрузитьКолонку("Стоимость"));
	...
	Иначе
		...
		РезультатРаспределенияНДС = РаспределитьПропорционально(НДС,Результат.ВыгрузитьКолонку("НДС"));
...
Процедура СФормироватьВводНачальныхОстатковНДСпоПартиям()
	...
	Пока ВыборкаПоОрганизациям.Следующий() Цикл
		...
		УстановитьНомерДокумента(ДокументВводНачальныхОстатковПоПартиям);
		КлючСтроки = 1;
