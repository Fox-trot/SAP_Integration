...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ОстаткиИОбороты(&НачИтоги, &ДатаКон, ДЕНЬ, Движения, Счет = &Счет, , Организация = &Организация) КАК ХозрасчетныйОстаткиИОбороты
	|
	|УПОРЯДОЧИТЬ ПО
	|	Период
	|ИТОГИ
	...
	Итог.УстановитьПараметр("Счет",Касса.СчетУчета);
	...
	|	КассовыеДокументы.Документ.Валюта КАК Валюта,
	...
	|			ТОГДА КассовыеДокументы.Документ.Выдать
	|		КОГДА КассовыеДокументы.Документ ССЫЛКА Документ.ПлатежнаяВедомость
	|			ТОГДА КассовыеДокументы.Документ.ВидРасчета.Наименование
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК ТекстДок,
	|	ВЫБОР
	|		КОГДА КассовыеДокументы.Документ ССЫЛКА Документ.ПриходныйКассовыйОрдер
	|			ТОГДА КассовыеДокументы.Документ.Сумма
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Приход,
	|	ВЫБОР
	|		КОГДА КассовыеДокументы.Документ ССЫЛКА Документ.РасходныйКассовыйОрдер
	|			ТОГДА КассовыеДокументы.Документ.Сумма
	|		КОГДА КассовыеДокументы.Документ ССЫЛКА Документ.ПлатежнаяВедомость
	|			ТОГДА КассовыеДокументы.Документ.Выплачено
	|		КОГДА КассовыеДокументы.Документ ССЫЛКА Документ.ПлатежнаяВедомостьКомандировочные
	|			ТОГДА КассовыеДокументы.Документ.Выплачено
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Расход,
	|	ВЫБОР
	|		КОГДА КассовыеДокументы.Документ ССЫЛКА Документ.РасходныйКассовыйОрдер
	|				ИЛИ КассовыеДокументы.Документ ССЫЛКА Документ.ПлатежнаяВедомость
	|				ИЛИ КассовыеДокументы.Документ ССЫЛКА Документ.ПлатежнаяВедомостьКомандировочные
	...
	|		И ПриходныйКассовыйОрдер.СчетКасса = &Счет
	...
	|		И РасходныйКассовыйОрдер.СчетКасса = &Счет
	...
	|	ГДЕ
	|		ПлатежнаяВедомость.Дата МЕЖДУ &НачИтоги И &ДатаКон
	|		И ПлатежнаяВедомость.Организация = &Организация
	|		И ПлатежнаяВедомость.Счет = &Счет
	|		И ПлатежнаяВедомость.Проведен
	|		И ПлатежнаяВедомость.ВидРасчета.Код <> &ПК
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ПлатежнаяВедомостьКомандировочные.Ссылка
	|	ИЗ
	|		Документ.ПлатежнаяВедомостьКомандировочные КАК ПлатежнаяВедомостьКомандировочные
	|	ГДЕ
	|		ПлатежнаяВедомостьКомандировочные.Дата МЕЖДУ &НачИтоги И &ДатаКон
	|		И ПлатежнаяВедомостьКомандировочные.Организация = &Организация
	|		И ПлатежнаяВедомостьКомандировочные.Счет = &Счет
	|		И ПлатежнаяВедомостьКомандировочные.Проведен
	|		И (НЕ ПлатежнаяВедомостьКомандировочные.Валютная)) КАК КассовыеДокументы
	...
	Запрос.УстановитьПараметр("Счет",Касса.СчетУчета);
 	Запрос.УстановитьПараметр("ПК",ПланыВидовРасчета.Удержания.ЗаработнаяПлатаНаПластиковуюКарточку.Код);
	...
	СведенияОбОрганизации = СведенияОЮрФизЛице(Организация, ,КонецДня(ДатаКон));

	
	ТипПКО=Тип("ДокументСсылка.ПриходныйКассовыйОрдер");
	ТипРКО=Тип("ДокументСсылка.РасходныйКассовыйОрдер");
	ТипПВ=Тип("ДокументСсылка.ПлатежнаяВедомость");
	ТипПВК=Тип("ДокументСсылка.ПлатежнаяВедомостьКомандировочные");
	...
	Пока ПоПроводкам Или ПоДокументам  Цикл
		...
		Если ДатаЛиста >= ДатаНач Тогда
			...
			ОбластьВкладнойЛистОтчет.Параметры.ЗаголовокЛиста="КАССА за "+Формат(ДатаЛиста, "ДФ=dd.MM.yyyy")+" по "+СокрЛП(Касса);
			ДокументРезультат.Вывести(ОбластьВкладнойЛистОтчет);
			
			ОбластьОтчетКассираОтчет.Параметры.ЗаголовокЛиста="КАССА за "+Формат(ДатаЛиста, "ДФ=dd.MM.yyyy")+" по "+СокрЛП(Касса);
...
//		ЛистовЗаГод = 1 стр
		...
		Если (ПоДокументам) И (Запрос.День = ДатаЛиста) Тогда
			...
			Пока ВыборкаДокумент.Следующий() Цикл
				...
				ТипДокумента=ТипЗнч(ВыборкаДокумент.Документ);
				
				Если ТипДокумента=ТипПКО Тогда
					НоваяСтрока.ВидДокумента="ПриходныйОрдер";
				ИначеЕсли ТипДокумента=ТипРКО Тогда
					НоваяСтрока.ВидДокумента="РасходныйОрдер";
				ИначеЕсли ТипДокумента=ТипПВ Тогда
					НоваяСтрока.ВидДокумента="ПлатежнаяВедомость";
				ИначеЕсли ТипДокумента=ТипПВК Тогда
					НоваяСтрока.ВидДокумента="ПлатежнаяВедомостьКомандировочные";
				КонецЕсли;	
				...
				Если (Не ЗначениеНеЗаполнено(ВыборкаДокумент.Валюта))и(Не ВыборкаДокумент.Валюта=Константы.ВалютаРегламентированногоУчета.Получить()) Тогда
		...
		Для Каждого СтрокаТаблица ИЗ РабочаяТаблица Цикл
			...
			СтруктураКурсов=ПолучитьКурсВалюты(СтрокаТаблица.Валюта,ДатаЛиста);
			...
			Если КурсоваяРазница <> 0 Тогда
				...
				Расход = 0;
				Если КурсоваяРазница > 0 Тогда
					КоррСчет = ПланыСчетов.Хозрасчетный.А9540;
		...
		Для Каждого Документ Из ТаблицаДокументы Цикл
			...
			Если Документ.ВидДокумента="ПриходныйОрдер" 
				...
				или Документ.ВидДокумента="ПлатежнаяВедомостьКомандировочные" Тогда
			...
			Если ЕстьВалюта Тогда
				...
				СтруктураКурсов=ПолучитьКурсВалюты(Валюта,Документ.ДатаДок);
			...
			Если Документ.ВидДокумента="РасходныйОрдер" Тогда
				...
				ВалПриход = 0;
				Приход = 0;
			ИначеЕсли Документ.ВидДокумента="ПлатежнаяВедомость" Тогда
				Клиент = "Выдано " + СокрЛП(Документ.ТекстДок) +" за "+Формат(Документ.Документ.МесяцВыплаты,"ДФ='MMMM yyyy ""г.""'");
				КоличествоРасходныхДокументов = КоличествоРасходныхДокументов + 1;
				ВалРасход = Документ.Расход;
				Расход = Окр(Документ.Расход*Курс/Кратность,2,1);
				ВалПриход = 0;
				Приход = 0;
			ИначеЕсли Документ.ВидДокумента="ПлатежнаяВедомостьКомандировочные" Тогда
				Клиент = "Выданы командировочные " + СокрЛП(Документ.ТекстДок) ;
				КоличествоРасходныхДокументов = КоличествоРасходныхДокументов + 1;
				ВалРасход = Документ.Расход;
				Расход = Окр(Документ.Расход*Курс/Кратность,2,1);
				ВалПриход = 0;
			...
			НомерДокПечатнойФормы = ПолучитьНомерНаПечать(СтруктураНомерДок);
		...
		Для Каждого Строка ИЗ РабочаяТаблица Цикл
			...
			СтруктураКурсов=ПолучитьКурсВалюты(Строка.Валюта,ДатаЛиста);
		...
		Если (ПоДокументам) И (ДатаЛиста = Запрос.День) Тогда
			ПоДокументам = Запрос.Следующий();
		КонецЕсли;
		// Проверим, если месяц закончился, то выведем кличество листов
		Если (ПоПроводкам И МЕСЯЦ(ДатаЛиста) <> МЕСЯЦ(Итог.Период))
			ИЛИ (ПоДокументам И МЕСЯЦ(ДатаЛиста) <> МЕСЯЦ(Запрос.День)) Тогда
