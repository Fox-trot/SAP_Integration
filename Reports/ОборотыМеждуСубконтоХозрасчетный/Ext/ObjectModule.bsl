...
Функция ПолучитьТекстЗапроса()
	...
	соотВидИтогов = Новый Соответствие;
	соотВидИтогов.Вставить(ТипИзмеренияПостроителяОтчета.Иерархия,       "ИЕРАРХИЯ");
	соотВидИтогов.Вставить(ТипИзмеренияПостроителяОтчета.ТолькоИерархия, "ТОЛЬКО ИЕРАРХИЯ");
	соотВидИтогов.Вставить(ТипИзмеренияПостроителяОтчета.Элементы,       "");
	...
	Для каждого стр Из ПостроительОтчета.ИзмеренияСтроки Цикл
		...
		|	" + стр.Имя + " " + соотВидИтогов.Получить(стр.ТипИзмерения);
	...
	+ ПоляВыбора + "
	|ИЗ
	|	РегистрБухгалтерии." + ИмяРегистраБухгалтерии + ".Обороты(&ДатаНач, &ДатаКон, Период, " + ПоляОтбораСчет + ", &МассивВидовСубконто, Организация = &Организация " + ПоляОтбора + ", " + ПоляОтбораКорСчет + ", &МассивКорВидовСубконто) КАК Обороты
	|ИТОГИ
	|	СУММА(СуммаОборотДт),
	|	СУММА(СуммаОборотКт),
...
Процедура УстановитьПараметрыЗапроса(Запрос)
	...
	УдалитьИзМассиваПустыеЭлементы(МассивВидовСубконто);
	УдалитьИзМассиваПустыеЭлементы(МассивКорВидовСубконто);
...
Процедура ЗаполнитьОбщуюРасшифровку(ДокументРезультат)
	...
	Расшифровка.Вставить("Организация", Организация);
	Расшифровка.Вставить("ДатаНач"    , ДатаНач);
	Расшифровка.Вставить("ДатаКон"    , ДатаКон);
...
Функция ПолучитьРасшифровку(ОтборДляРасшифровки, БазоваяРасшифровка=Неопределено)
	...
	Если БазоваяРасшифровка <> Неопределено Тогда
		Расшифровка = ПолучитьКопиюСтруктуры(БазоваяРасшифровка);
	Иначе
		Расшифровка = Новый Структура;
	КонецЕсли;
...
Процедура ВывестиГруппировку(Выборка, Знач КолвоГруппировок, ИмяГруппировки="", ДокументРезультат, ОблСубконто, ОблДетали, ОтборДляРасшифровки)
	
	Если КолвоГруппировок=0 Тогда
		// детальные записи
		ВыборкаДетали = Выборка.Выбрать(ОбходРезультатаЗапроса.Прямой);
		
		СтруктураДляРасшифровки = ПолучитьКопиюСтруктуры(ОтборДляРасшифровки);
		...
		Пока ВыборкаДетали.Следующий() Цикл
			...
			ОблДетали.Параметры.ВалютаДт = ?(Не ЗначениеНеЗаполнено(ВыборкаДетали.ВалютнаяСуммаОборотДт), ВыборкаДетали.ВалютаПредставление, "");
			ОблДетали.Параметры.ВалютаКт = ?(Не ЗначениеНеЗаполнено(ВыборкаДетали.ВалютнаяСуммаОборотКт), ВыборкаДетали.ВалютаПредставление, "");
			
			ОблДетали.Параметры.Расшифровка = ПолучитьРасшифровку(СтруктураДляРасшифровки, БазоваяРасшифровка);
			
			ДокументРезультат.Вывести(ОблДетали, ПостроительОтчета.ИзмеренияСтроки.Количество());
		
		КонецЦикла;
		
	Иначе
		// групповые записи
		СтрокаИтогов = ПостроительОтчета.ИзмеренияСтроки[ПостроительОтчета.ИзмеренияСтроки.Количество()-КолвоГруппировок];
		ПсевдонимПоля = СтрокаИтогов.Имя;
		
		Если ЗначениеНеЗаполнено(ИмяГруппировки) Тогда
			ВыборкаСубконто = Выборка.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, ПсевдонимПоля);
		Иначе
			ВыборкаСубконто = Выборка.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, ПсевдонимПоля, ИмяГруппировки);
		КонецЕсли;
		...
		Пока ВыборкаСубконто.Следующий() Цикл
			...
			ОблСубконто.Параметры.Расшифровка = ПолучитьРасшифровку(СтруктураДляРасшифровки);
			
			ДокументРезультат.Вывести(ОблСубконто, ПостроительОтчета.ИзмеренияСтроки.Количество() - КолвоГруппировок);
			
			ИмяГруппировки = ВыборкаСубконто.Группировка();
			
			ВывестиГруппировку(ВыборкаСубконто, КолвоГруппировок-1, ИмяГруппировки, ДокументРезультат, ОблСубконто, ОблДетали, СтруктураДляРасшифровки);
...
Процедура ВывестиОтчет(ДокументРезультат, Макет)
	...
	КолвоГруппировок = ПостроительОтчета.ИзмеренияСтроки.Количество();
	
	ДокументРезультат.НачатьАвтогруппировкуСтрок();
	
	ВывестиГруппировку(ВыборкаОбщие, КолвоГруппировок, , ДокументРезультат, ОблСубконто, ОблДетали, Новый Структура);
...
//
Процедура УдалитьИзМассиваПустыеЭлементы(МассивЭлементов)

	Колво = МассивЭлементов.Количество();
	Для н=1 По Колво Цикл
		Если ЗначениеНеЗаполнено(МассивЭлементов[Колво-н]) Тогда
	...
	Если ТипЗнч(СтрокиОтбора) = Тип("Соответствие")
		...
		Для каждого ЭлементОтбора Из СтрокиОтбора Цикл
			...
			Если ЭлементОтбора.Ключ = "Валюта" Тогда
				...
				ПоВалюте = Не ЗначениеНеЗаполнено(Валюта);
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Для н=0 По Колво Цикл
		Если ЗначениеНеЗаполнено(МассивВидовСубконто[Колво-н]) Тогда
	...
	Для н=0 По Колво Цикл
		Если ЗначениеНеЗаполнено(МассивКорВидовСубконто[Колво-н]) Тогда
...
//
// Параметры
//  Струк  – Структура или Соответствие – объект копирования
//
// Возвращаемое значение:
//   Структура или Соответствие   – копия объекта
//
Функция ПолучитьКопиюСтруктуры(Струк)

	Если ТипЗнч(Струк) <> Тип("Структура")
		и ТипЗнч(Струк) <> Тип("Соответствие") Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Копия = ?(ТипЗнч(Струк) = Тип("Структура"), Новый Структура, Новый Соответствие);
	
	Для каждого Элемент Из Струк Цикл
