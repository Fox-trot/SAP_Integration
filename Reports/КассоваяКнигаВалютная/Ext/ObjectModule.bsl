...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	|				"+?(ВалютаОтбор.Пустая(),""," И Валюта = &ВалютаОтбор")+") КАК ХозрасчетныйОстаткиИОбороты
	|
	|УПОРЯДОЧИТЬ ПО
	|	Период
	|ИТОГИ
	...
	Итог.УстановитьПараметр("ВалютаОтбор",ВалютаОтбор);
	...
	|			ТОГДА КассовыеДокументы.Документ.Валюта
	|		ИНАЧЕ КассовыеДокументы.Документ.ВалютаДокумента
	|	КОНЕЦ КАК Валюта,
	|	КассовыеДокументы.Документ.Основание КАК Основание,
	|	ВЫБОР
	|		КОГДА КассовыеДокументы.Документ ССЫЛКА Документ.ПриходныйКассовыйОрдерВалютный
	...
	|	КОНЕЦ КАК Приход,
	|	ВЫБОР
	|		КОГДА КассовыеДокументы.Документ ССЫЛКА Документ.РасходныйКассовыйОрдерВалютный
	|			ТОГДА КассовыеДокументы.Документ.Сумма
	|		КОГДА КассовыеДокументы.Документ ССЫЛКА Документ.ПлатежнаяВедомостьКомандировочные
	...
	|	ИЗ
	|		Документ.ПриходныйКассовыйОрдерВалютный КАК ПриходныйКассовыйОрдер
	|	ГДЕ
	...
	|		И ПриходныйКассовыйОрдер.ВалютаДокумента = &ВалютаОтбор
	|		"+?(ВалютаОтбор.Пустая(),""," И ПриходныйКассовыйОрдер.ВалютаДокумента = &ВалютаОтбор")+"
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ИЗ
	|		Документ.РасходныйКассовыйОрдерВалютный КАК РасходныйКассовыйОрдер
	|	ГДЕ
	...
	|		"+?(ВалютаОтбор.Пустая(),""," И РасходныйКассовыйОрдер.ВалютаДокумента = &ВалютаОтбор")+"
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		И ПлатежнаяВедомостьКомандировочные.Валютная) КАК КассовыеДокументы
	...
	Запрос.УстановитьПараметр("ВалютаОтбор",ВалютаОтбор);
	...
	ТипПКО=Тип("ДокументСсылка.ПриходныйКассовыйОрдерВалютный");
	ТипРКО=Тип("ДокументСсылка.РасходныйКассовыйОрдерВалютный");
...
//		ЛистовЗаГод = 1 стр
		...
		Если (ПоДокументам) И (Запрос.День = ДатаЛиста) Тогда
			...
			Пока ВыборкаДокумент.Следующий() Цикл
				...
				НоваяСтрока.ВидДокумента=?(ТипЗнч(ВыборкаДокумент.Документ)=ТипПКО,"ПриходныйОрдерВалютный","РасходныйОрдерВалютный");
		...
		Для Каждого СтрокаТаблица ИЗ РабочаяТаблица Цикл
			...
			Если КурсоваяРазница <> 0 Тогда
				Если ПерваяСтрока = 1 Тогда
					...
					ПерваяСтрока = 0;
				КонецЕсли;
				ВалютаСтроки = СтрокаТаблица.Валюта;
				Приход = 0;
				Расход = 0;
				Если КурсоваяРазница > 0 Тогда
				...
				Если ДатаЛиста >= ДатаНач Тогда
					...
					ОбластьКурсовыеРазницыПоВалютеОтчет.Параметры.НадписьКРВалюта="по "+ВалютаСтроки.Наименование;
		...
		Для Каждого Документ Из ТаблицаДокументы Цикл
			...
			Если Документ.ВидДокумента="ПриходныйОрдерВалютный" или Документ.ВидДокумента="РасходныйОрдерВалютный" Тогда
			...
			Если Документ.ВидДокумента="РасходныйОрдерВалютный" или Документ.ВидДокумента="ПлатежнаяВедомостьКомандировочные" Тогда
			...
			Если ДатаЛиста >= ДатаНач Тогда
				Если ЕстьВалюта Тогда
					Если ВыводитьОснования Тогда
						...
						ОбластьСтрокаВалШирокаяОтчет.Параметры.ВалПриход=Формат(ВалПриход,"ЧЦ=15; ЧДЦ=2; ЧРД==")+" "+?(ВалПриход<>0,Валюта.Наименование,"");
						ОбластьСтрокаВалШирокаяОтчет.Параметры.ВалРасход=Формат(ВалРасход,"ЧЦ=15; ЧДЦ=2; ЧРД==")+" "+?(ВалРасход<>0,Валюта.Наименование,"");
					...
					Иначе
						...
						ОбластьСтрокаВалОтчет.Параметры.ВалПриход=Формат(ВалПриход,"ЧЦ=15; ЧДЦ=2; ЧРД==")+" "+?(ВалПриход<>0,Валюта.Наименование,"");
						ОбластьСтрокаВалОтчет.Параметры.ВалРасход=Формат(ВалРасход,"ЧЦ=15; ЧДЦ=2; ЧРД==")+" "+?(ВалРасход<>0,Валюта.Наименование,"");
