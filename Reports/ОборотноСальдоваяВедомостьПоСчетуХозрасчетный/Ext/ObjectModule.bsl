...
Функция СформироватьЗаголовок() Экспорт
	...
	Если Счет.Валютный и не Валюта.Пустая() Тогда
		СтрОтбор="Валюта: "+Валюта.Наименование+"; "+СтрОтбор;  
	КонецЕсли;	
...
Процедура ВывестиГруппировку(Выборка, Знач ИндексТекущейГруппировки, МассивГруппировок, СдвигУровня, 
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Лев(Выборка.Группировка(), 8) = "Субконто" Тогда
			...
			//Если ТипЗнч(Выборка[Выборка.Группировка()]) = Тип("СправочникСсылка.Контрагенты") Тогда
			//	ВыводимаяОбласть.Параметры.СубконтоПредставление = "Код: "+СокрЛП(Выборка[Выборка.Группировка()+"Код"])+Символы.ПС+"ИНН: "+СокрЛП(Выборка[Выборка.Группировка()+"ИНН"])+Символы.ПС+Выборка[Выборка.Группировка()+"Представление"];
			//ИначеЕсли ТипЗнч(Выборка[Выборка.Группировка()]) = Тип("СправочникСсылка.Номенклатура")
			//или ТипЗнч(Выборка[Выборка.Группировка()]) = Тип("СправочникСсылка.НоменклатураПродаж") Тогда
			//	ВыводимаяОбласть.Параметры.СубконтоПредставление = "Код: "+СокрЛП(Выборка[Выборка.Группировка()+"Код"])+Символы.ПС+Выборка[Выборка.Группировка()+"Представление"];
			//Иначе
...
Функция ВыполнитьЗапросОтчета()
	...
	Для каждого Измерение Из ПостроительОтчета.ИзмеренияСтроки Цикл
		...
		Если Измерение.Представление="Номенклатура" 
			или Измерение.Представление="Номенклатура продаж" Тогда
			ТекстПоля = ТекстПоля + ", " + Измерение.ПутьКДанным + ".Код КАК " + Измерение.Имя+"Код";
		ИначеЕсли Измерение.Представление="Контрагенты" Тогда
			ТекстПоля = ТекстПоля + ", " + Измерение.ПутьКДанным + ".Код КАК " + Измерение.Имя+"Код";
			ТекстПоля = ТекстПоля + ", " + Измерение.ПутьКДанным + ".ИНН КАК " + Измерение.Имя+"ИНН";
		КонецЕсли;
	...
	Если Счет.Валютный и не Валюта.Пустая() Тогда
		ТекстОтбор=ТекстОтбор+" И Валюта=&Валюта ";
	...
	|УПОРЯДОЧИТЬ ПО Субконто1.Наименование ";
	//|";
...
Процедура СформироватьОтчет(ДокументРезультат,  ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	СтруктураПараметров.Вставить("Субконто5Начало", Макет.ПолучитьОбласть("Субконто5|Начало"));
	...
	СтруктураПараметров.Вставить("ВалютаСубконто5Начало", Макет.ПолучитьОбласть("ВалютаСубконто5|Начало"));
	...
	СтруктураПараметров.Вставить("Субконто5КоличествоНачало", Макет.ПолучитьОбласть("Субконто5Количество|Начало"));
	...
	СтруктураПараметров.Вставить("Субконто5Числа", Макет.ПолучитьОбласть("Субконто5|Числа"));
	...
	СтруктураПараметров.Вставить("ВалютаСубконто5Числа", Макет.ПолучитьОбласть("ВалютаСубконто5|Числа"));
	...
	СтруктураПараметров.Вставить("Субконто5КоличествоЧисла", Макет.ПолучитьОбласть("Субконто5Количество|Числа"));
	...
	ВыводитьИтогПоВалюте=ложь;
	
	Если Счет.Валютный и не Валюта.Пустая() Тогда
		ВыводитьИтогПоВалюте=Истина;
	КонецЕсли;	
	...
	Для Каждого ИмяПоказателя Из МассивПоказателей Цикл
		...
		Если не ВыводитьИтогПоВалюте Тогда
			Если ИмяПоказателя<>"Сумма" и ИмяПоказателя<>"Количество" Тогда
				Продолжить;
			КонецЕсли;	
		КонецЕсли;
		...
		Если ВыводитьИтогПоВалюте и ИмяПоказателя="ВалютнаяСумма" Тогда
			Область = Макет.ПолучитьОбласть("ИтогиСтрока|Числа");
		ИначеЕсли ИмяПоказателя="Количество" Тогда
			Область = Макет.ПолучитьОбласть("ИтогиСтрока|Числа");
		Иначе	
...
Функция ИмяСекцииПоГруппировке(ИмяГруппировки)
	...
	Если Лев(ИмяГруппировки,8)="Субконто" Тогда
		...
		Возврат "Субконто" + Строка(ПостроительОтчета.ИзмеренияСтроки.Индекс(ПостроительОтчета.ИзмеренияСтроки.Найти(ИмяГруппировки))+1);
	Иначе
