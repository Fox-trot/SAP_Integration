...
Процедура ЗаполнитьПоляВСоответствииСоСпискомИнтервалов() Экспорт
	...
	Для Каждого Интервал Из СписокИнтервалов Цикл
		...
		Поле.ТипЗначения = ПолучитьОписаниеТиповЧисла(15, 2);
		ТиповыеОтчеты.УстановитьПараметр(Поле.Оформление, "Формат", "ЧЦ=15; ЧДЦ=2");
...
Процедура УстановитьТекстЗапроса()
	...
	Если НЕ ЗначениеЗаполнено(Организация) Тогда
		ТекстЗапросаПоОстаткам = СтрЗаменить(ТекстЗапросаПоОстаткам, "Организация = &Организация", "");
	КонецЕсли;
...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = "Кредиторская задолженность по срокам долга";
