...
Функция СформироватьЗапрос(СтруктураПараметров)
	...
	+ ?(ЗначениеНеЗаполнено(Организация), "",	" Организация = &Организация") + ") КАК ОстаткиИОбороты";
...
Функция СформироватьТаблицуРазворотаСчетов(СтруктураПараметров)
	...
	Если ПравилаВыводаИтогов.Количество() > 0 тогда
		...
		Для Каждого ПравилаСчета из ПравилаВыводаИтогов Цикл
			...
			МожноИспользоватьСоставнойЗапрос = Истина; // признак того, что можно использовать запрос 
			// со вложенным запросом, ограничивающим типы полей субконто
			...
			Для каждого СтрокаПоСубконто Из ПравилаСчета.РазворотПоСубконто Цикл
				...
				Для каждого ВидСубконто Из ПравилаСчета.Счет.ВидыСубконто Цикл
					...
					КонецЕсли;
				КонецЦикла;
				МожноИспользоватьСоставнойЗапрос = МожноИспользоватьСоставнойЗапрос И (ВидСубконто.ВидСубконто.ТипЗначения.Типы().Количество()<=10);
			КонецЦикла;
			...
			Для каждого СтрокаПоСубконто Из ПравилаСчета.РазворотПоСубконто Цикл
				...
				Если ЕстьПредставление 
					И МожноИспользоватьСоставнойЗапрос
					Тогда
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Уровень() = 0 Тогда
			ОбластьСтрокаСчет = Макет.ПолучитьОбласть("Строка|Счет");
			СтруктураПараметров.Вставить("ОбластьСтрокаЧисла", Макет.ПолучитьОбласть("Строка|Числа"));
		ИначеЕсли Выборка.Уровень() = 1 Тогда
			ОбластьСтрокаСчет = Макет.ПолучитьОбласть("Строка1|Счет");
			СтруктураПараметров.Вставить("ОбластьСтрокаЧисла", Макет.ПолучитьОбласть("Строка1|Числа"));
		Иначе
			ОбластьСтрокаСчет = Макет.ПолучитьОбласть("Строка2|Счет");
			СтруктураПараметров.Вставить("ОбластьСтрокаЧисла", Макет.ПолучитьОбласть("Строка2|Числа"));
		КонецЕсли;
		ОбластьСтрокаСчет.Параметры.Заполнить(Выборка);
		...
		Если ПоВалютам и Выборка.Счет.Валютный Тогда
