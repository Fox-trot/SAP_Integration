...
Функция ЗаголовокОтчета() Экспорт
	Возврат  "Анализ счета " + ?(НЕ ЗначениеНеЗаполнено(Счет), Счет, " не выбран счет");
КонецФункции
...
Функция СформироватьЗаголовок() Экспорт
	...
	Если НЕ ПоСубсчетамИСубконто И ПостроительОтчета.ИзмеренияСтроки.Количество()=0 И ЗначениеНеЗаполнено(Период) И НЕ ПоВалютам Тогда
...
Процедура ВывестиКорСчета(ВыборкаОбороты, СтруктураПараметров, ОтборДляРасшифровки)
	...
	Пока ВыборкаОборотыПоКорСчетам.Следующий() Цикл
		...
		Если ВыборкаОборотыПоКорСчетам.КорСчет = NULL И ЗначениеНеЗаполнено(ВыборкаОборотыПоКорСчетам.СуммаОборотДт) И ЗначениеНеЗаполнено(ВыборкаОборотыПоКорСчетам.СуммаОборотКт) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ВывестиВалюты(Выборка, СтруктураПараметров, ОтборДляРасшифровки, ТолькоОборотыПредУровень = Ложь)
	...
	ЕстьПериоды = НЕ ЗначениеНеЗаполнено(Период);
...
Процедура ОбработкаРасшифровкиСтандартногоОтчета(Расшифровка) Экспорт
	...
	ИначеЕсли Способрасшифровки="Журнал" Тогда
		...
		ФормаЖурнала.Отбор.Организация.Использование = НЕ ЗначениеНеЗаполнено(Организация);
