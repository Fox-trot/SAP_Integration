...
Процедура venkonЗаполнитьТабличныйДокументАктВыполненныхРабот(ТабличныйДокумент, ДанныеПечати, ОбластьФорматСтроки)
	...
	Если ДанныеЗаполненияШапки.Свойство("Валюта") Тогда
		...
		Если ВалютаДокумента <> мВалютаРегламентированногоУчета Тогда
			Если ТипЗнч(ДанныеЗаполненияШапки.ДатаДокумента) = Тип("Дата") Тогда
				СтруктураКурсов = ОбщегоНазначения.ПолучитьКурсВалюты(ВалютаДокумента, ДанныеЗаполненияШапки.ДатаДокумента);
			Иначе
				СтруктураКурсов = ОбщегоНазначения.ПолучитьКурсВалюты(ВалютаДокумента, Дата(1, 1, 1));
			КонецЕсли;
		КонецЕсли;	
	КонецЕсли;
	...
	Иначе
		Если ВалютаДокумента <> Неопределено Тогда
			Если ВалютаДокумента <> мВалютаРегламентированногоУчета Тогда
				ИтоговаяСуммаАкта = ИтоговаяСуммаАкта + НСтр("ru=', в т.ч. НДС: ';uz=', шу ж. ҚҚС:'") + Формат(ОбщегоНазначения.ПересчитатьИзВалютыВВалюту(СуммаНДСИтого, ВалютаДокумента, мВалютаРегламентированногоУчета, СтруктураКурсов.Курс,1, СтруктураКурсов.Кратность, 1),"ЧДЦ=2");
			Иначе
	...
	Если ВалютаДокумента <> Неопределено Тогда
		Если ВалютаДокумента <> мВалютаРегламентированногоУчета Тогда
			ЭквивалентнаяСумма = ОбщегоНазначения.ПересчитатьИзВалютыВВалюту(СуммаИтого, ВалютаДокумента, мВалютаРегламентированногоУчета, СтруктураКурсов.Курс, 1, СтруктураКурсов.Кратность, 1);
