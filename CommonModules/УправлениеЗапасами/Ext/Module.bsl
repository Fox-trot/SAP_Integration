	...
	Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
		...
		Если Не ЗначениеНеЗаполнено(СтрокаТаблицы.Номенклатура)
		   И  СтрокаТаблицы.Услуга Тогда
				...
				ОшибкаПриПроведении("В строке номер """+ СокрЛП(СтрокаТаблицы.НомерСтроки) +
...
Функция ЭтоИмпортныйТовар(СчетУчета, НомерГТД, СтранаПроисхождения) Экспорт
	...
	Если ЗначениеНеЗаполнено(СтранаПроисхождения) или
		 СтранаПроисхождения = Справочники.КлассификаторСтранМира.РОССИЯ или
		 ЗначениеНеЗаполнено(НомерГТД) Тогда
...
Функция НеУказанНомерГТД(НомерГТД, СтранаПроисхождения) Экспорт
	...
	ИначеЕсли ЗначениеНеЗаполнено(НомерГТД) Тогда
		Результат = Истина;
...
Процедура ПроведениеДокументаСписанияПоСчетуГТД(Документ, ТаблицаПоТоварам) Экспорт
	...
	Для каждого ТоварПоНомеруГТД из ТоварыПоНомерамГТД Цикл
		...
		Если КоличествоВПроводке < ТоварПоНомеруГТД.Количество Тогда
			...
			ОшибкаПриПроведении(СтрокаНачалаСообщенияОбОшибке + СтрокаСообщения,,);
		КонецЕсли;
		...
		Если КоличествоВПроводке > 0 Тогда
			...
			УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Номенклатура", ТоварПоНомеруГТД.Номенклатура);
			УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "НомераГТД", ТоварПоНомеруГТД.НомерГТД);
			УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "СтраныПроисхождения", ТоварПоНомеруГТД.СтранаПроисхождения);
