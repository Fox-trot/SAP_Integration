...
Процедура ПроверитьОстаткиНоменклатурыПродаж(Отказ,ДокументОбъект,ТаблицаНоменклатурыПродаж, Склад,Организация,ИмяСчетаУчета="",ДокументРезервирования=Неопределено) Экспорт
	...
	Для Каждого СтрокаНоменклатуры Из ТаблицаНоменклатурыПродаж  Цикл
		...
		Если (СтрокаОстатков.КоличествоОстатокДт-СтрокаОстатков.Резерв) < Количество Тогда
			Сообщить(Строка(ДокументОбъект) + " Недостаточно свободных остатков ("+СтрокаНоменклатуры.Номенклатура.Код+") """+СтрокаНоменклатуры.Номенклатура+""", счет учета "+СтрокаНоменклатуры[ИмяСчетаУчета].Код+", склад """+Склад.Наименование+"""",СтатусСообщения.Важное);
			Сообщить("В остатке: "+Строка(СтрокаОстатков.КоличествоОстатокДт)+" "+СтрокаНоменклатуры.Номенклатура.БазоваяЕдиницаИзмерения + " В резерве: "+Строка(СтрокаОстатков.Резерв)+" "+СтрокаНоменклатуры.Номенклатура.БазоваяЕдиницаИзмерения, СтатусСообщения.Важное);
...
Функция ПолучитьОстатокНоменклатуры(Номенклатура,Склад,Организация,МоментВремени=неопределено,Счет,ДатаПоступления=Неопределено) Экспорт
	...
	ТаблицаОстатков = Запрос.Выполнить().Выбрать();
	Остаток = Новый Структура("Количество,Сумма", 0, 0);
	Если ТаблицаОстатков.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(Остаток, ТаблицаОстатков);
	КонецЕсли;
	Возврат Остаток;
КонецФункции	 
