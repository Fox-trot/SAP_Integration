...
Процедура venkonЗаполнитьДанныеПоСчетФактуреГостВерсия40(ДеревоДанных, ДанныеДляФормированияЭД, СтруктураЭД)
	...
	ОбщегоНазначенияЭД.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных, "ROOT.Expansion.CurrencyCode",   ЭлектронныеДокументыБП.ПолучитьЗначениеРеквизита(ДанныеДляФормированияЭД.Валюта, "Код"));
...
Функция НайтиПодчиненныйСчетФактуру(Знач ДокументСсылка, ВидДокумента = "СчетФактураВыданный", Отбор = Неопределено, ИсключитьИзВыборкиСФ = Неопределено, МетаданныеОснования = Неопределено) Экспорт
	...
	Иначе
		...
		Если ЕстьРеквизитДокумента("ИспользоватьДокументРасчетовКакСчетФактуру", МетаданныеОснования) 
			И ЕстьРеквизитДокумента("РасчетныйДокумент", МетаданныеОснования) 
		...
		Если ВидДокумента = "СчетФактураПолученный" 
			И ЕстьРеквизитДокумента("ПредъявленСчетФактура", МетаданныеОснования)
...
Функция ФорматСуммЭД(СуммаКПрописи, КодВалюты = Неопределено, ЧН = "", ЧРГ = "") Экспорт
	...
	Возврат ОбщегоНазначения.ФорматСуммЭД(СуммаКПрописи, Валюта,  ЧН, ЧРГ);
...
Процедура РазобратьНаименованиеФизЛица(ПолноеНаименование, Фамилия = " ", Имя = " ", Отчество = " ") Экспорт
	...
	ФамилияИнициалыФизЛица(ПолноеНаименование,Фамилия,Имя,Отчество);
