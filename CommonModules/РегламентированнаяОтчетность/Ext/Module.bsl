...
Функция ОчиститьТелефонныйНомерОтРазделителей(ИсходныйНомер)
	Перем Результат;
	
	Результат = ИсходныйНомер;
	
	Для Индекс = 1 По СтрДлина(ИсходныйНомер) Цикл
		КодСимв = КодСимвола(Сред(ИсходныйНомер,Индекс,1));
...
Функция РазложитьСериюДокумента(Знач ВидДокумента, Знач СерияДокумента) Экспорт
	...
	Если ЗначениеЗаполнено(ВидДокумента) тогда
		...
		Если (КодДока = "01") или (КодДока = "03")  Тогда
			...
			Часть1 = ?(Разделитель = 0, СерияДокумента, РегламентированнаяОтчетность.ЗаменитьОдниСимволыДругими("1УХЛС", ВРег(СокрЛП(Лев(СерияДокумента, Разделитель-1))), "IVXLC"));
...
Функция ПреобразоватьЧислоВРимскуюНотацию(АрабскоеЧисло, ИспЛатинскиеБуквы = Ложь) Экспорт
	...
	РимскоеЧисло="";
	cRab = РегламентированнаяОтчетность.ДополнитьСтроку(АрабскоеЧисло,3);
...
Функция ИДКонфигурации() Экспорт
	...
	Если МетаданныеИмя = "БухгалтерияДляУзбекистана" ИЛИ МетаданныеИмя = "БухгалтерияПредприятияБазовая" Тогда
		Возврат "БП";
...
Функция ПолучитьСУчетомКэша(Кэш, Параметр, ФизЛицо = Неопределено)
	...
	ИначеЕсли Параметр = "СведенияОРуководителеГражданство" 
...
Функция АдресВФормате9Запятых(Запись)
	...
	Представление = ПолучитьПредставлениеАдреса(Запись);
...
Функция ПредставлениеАдресаВФормате9Запятых(Знач АдресВФормате9Запятых, АнализироватьРегион = Ложь) Экспорт
	...
	Если СокрЛП(СоставляющиеАдреса[2]) <> "" Тогда
		...
		Если АнализироватьРегион Тогда
			Если ТолькоЦифрыВСтроке(Регион) Тогда
...
//                   
Функция ПолучитьСведенияОбОрганизации(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено, Знач ДатаНачалаПериодаОтчета = Неопределено, Знач ДатаКонцаПериодаОтчета = Неопределено) Экспорт
	...
	Если Организация = Неопределено ИЛИ Организация = ПустоеЗначениеТипа("СправочникСсылка.Организации") Тогда
		Возврат ОргСведения;
	КонецЕсли;
	...
	Если СписокПоказателей = Неопределено Тогда
		...
		СписокПоказателей.Добавить("КодОКЭД");
		...
		СписокПоказателей.Добавить("Численность", "Численность");
		СписокПоказателей.Добавить("ЧисленностьНа", "ЧисленностьНа");
		СписокПоказателей.Добавить("Микрофирма", "Микрофирма");
		СписокПоказателей.Добавить("Площадь", "Площадь");
	...
	Для Каждого Показатель Из СписокПоказателей Цикл
		...
		ИначеЕсли Имяпоказателя = "НаимНО" Тогда
			...
			Значение = Организация.ГНИОрганизации;
...
//			Значение = Организация.РегистрационныйНомерФСС;
		...
		ИначеЕсли ИмяПоказателя = "КодОКЭД" Тогда
			
			Значение = Организация.КодОКЭД;
		...
		ИначеЕсли ИмяПоказателя = "СвидНО" Тогда
			...
			Значение = Организация.НомерСвидетельстваОГосРегистрации;
	...
	//		Значение = Организация.КПП;
		...
		ИначеЕсли ИмяПоказателя = "АдрЮР" Тогда
			...
			Данные = Новый Структура("Объект, Тип, Вид",Организация.Ссылка, Перечисления.ТипыКонтактнойИнформации.Адрес, Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации);
		...
		ИначеЕсли ИмяПоказателя = "ТелОрганизации" Тогда
			...
			Если Результат <> Неопределено Тогда
				Значение = ОчиститьТелефонныйНомерОтРазделителей(Результат.Представление);
			Иначе
				Значение = "";
			КонецЕсли;
		...
		ИначеЕсли ИмяПоказателя = "ФаксОрганизации" Тогда
			...
			Если Кэш.ПБОЮЛ Тогда
				Значение = "";
			Иначе
				Данные = Новый Структура("Объект, Тип, Вид", Кэш.Организация.Ссылка, Перечисления.ТипыКонтактнойИнформации.Телефон, Справочники.ВидыКонтактнойИнформации.ФаксОрганизации);
				Результат = РегистрыСведений.КонтактнаяИнформация.Получить(Данные);
				Если Результат <> Неопределено Тогда
					Значение = Результат.Представление;
				Иначе
		...
		ИначеЕсли ИмяПоказателя = "ФИОРук" Тогда
			...
			Руководители = ОтветственныеЛица(Организация.Ссылка, ДатаЗначения);	
			
			Значение=Руководители.РуководительПредставление;   
			
		ИначеЕсли ИмяПоказателя = "ИННРук" Тогда
			
			Руководители = ОтветственныеЛица(Организация.Ссылка, ДатаЗначения);	
			
			Значение=Руководители.Руководитель.ФизическоеЛицо.ИНН;   
		...
		ИначеЕсли ИмяПоказателя = "ФИОБух" ИЛИ ИмяПоказателя = "ФИООтпр" Тогда
			...
			Значение=Руководители.ГлавныйБухгалтерПредставление;   

		ИначеЕсли ИмяПоказателя = "ИННБух" Тогда
			
			Руководители = ОтветственныеЛица(Организация.Ссылка, ДатаЗначения);	
			
			Значение=Руководители.ГлавныйБухгалтер.ФизическоеЛицо.ИНН;   
		...
		ИначеЕсли ИмяПоказателя = "Численность" Тогда
			
			Если ДатаНачалаПериодаОтчета = Неопределено ИЛИ ДатаКонцаПериодаОтчета = Неопределено Тогда
				Значение = ПолучитьЧисленностьСотрудниковОрганизации(Организация.Ссылка, НачалоМесяца(ДатаЗначения), КонецМесяца(ДатаЗначения));
			Иначе
				НачПериода = НачалоМесяца(ДатаНачалаПериодаОтчета);
				КонПериода = КонецМесяца(НачПериода);
				кМ=0;	кС=0;
				Пока КонПериода <= КонецМесяца(ДатаКонцаПериодаОтчета) Цикл
					кМ = кМ + 1;
					кС = кС + ПолучитьЧисленностьСотрудниковОрганизации(Организация.Ссылка, НачПериода, КонПериода);
					НачПериода = КонПериода+1;
					КонПериода = КонецМесяца(НачПериода);
				КонецЦикла;
				Значение = ?(кМ = 0, 0, Формат(Окр(кС/кМ,0),"ЧГ="));
			КонецЕсли;
				
		ИначеЕсли ИмяПоказателя = "ЧисленностьНа" Тогда
			
			Значение = НалоговаяОтчетность.ЧисленностьРаботников(Организация.Ссылка,ДатаКонцаПериодаОтчета);
			
		ИначеЕсли ИмяПоказателя = "Микрофирма" Тогда
			
			Значение = "";//СтруктураУчетнойПолитики(ДатаЗначения, Организация.Ссылка).РазмерПредприятия;
				
		ИначеЕсли ИмяПоказателя = "Площадь" Тогда
			
			ЗанимаемыеПомещения = Организация.ЗанимаемыеПомещения;
			Значение = ЗанимаемыеПомещения.Итог("Площадь");
	...
	Если Организация <> Неопределено тогда
		...
		Если ИДКонфигурации() = "ЗУП" Тогда
			...
			Если Исполнитель <> Неопределено тогда
				...
				ЗапросПоИсполнителям.УстановитьПараметр("ГоловнаяОрганизация", ГоловнаяОрганизация(Организация));
		...
		ИначеЕсли (ИДКонфигурации() = "БП" И РедакцияКонфигурации() > "1.0") Тогда
			ЗапросПоЛицам = Новый Запрос();
			ЗапросПоЛицам.УстановитьПараметр("Организация", Организация);
			ЗапросПоЛицам.УстановитьПараметр("ДатаСреза",   ДатаСреза);
			ЗапросПоЛицам.Текст = 
		...
		Иначе
			...
			Если (ИДКонфигурации() = "УПП" И РедакцияКонфигурации() > "1.1") Тогда
				ЗапросПоЛицам = Новый Запрос();
				ЗапросПоЛицам.УстановитьПараметр("Организация", Организация);
				ЗапросПоЛицам.УстановитьПараметр("ДатаСреза",   ДатаСреза);
...
Функция РазбитьПоПодразделам(ИмяРаздела, мТаблицаСоставПоказателей, МногострочныеИсключения = Неопределено) Экспорт
	...
	Для Каждого Стр из СтрокиФорматаТекРаздела Цикл
		...
		ИначеЕсли (Многостр = ПослМногостр) и (МногоСтр = "/") и (Лев(ПослКодПоказателя, СтрДлина(ПослКодПоказателя)-2) <> Лев(КодПоказателя, СтрДлина(КодПоказателя)-2))
...
Функция ВывестиНаПечатьЛистРаздела(Форма, СтруктураПараметровЛиста)
	...
	ОсобыйСлучай = 0;
	//Особый случай вывода на печать многострочной части регл.отчета
	// шапка выводится только на первой странице многострочного раздела
	// подвал выводиться только на последней странице многострочного раздела
	
	Если Найти(""  +Форма.ОбработкаОбъект.ЭтотОбъект,"РегламентированныйОтчетНДС") > 0 И ИмяРаздела = "Приложение7" Тогда
			ОсобыйСлучай = 1;
	КонецЕсли;
	...
	КолВоЛистов = 0;
	ОсталосьСтрокКВыводуНаПечать = МаксимальноеКоличествоОбластейМногострочнойЧасти(МногострочныеЧасти);
	Пока МаксимальноеКоличествоОбластейМногострочнойЧасти(МногострочныеЧасти) > 0 Цикл
		...
		Для Каждого Эл Из МногострочныеЧасти Цикл
			...
			Инд = Инд + 1;
		КонецЦикла;
		Если ОсобыйСлучай = 1 Тогда    //
			Если СписокПечатныхФорм.Количество() = 3 Тогда //
		...
		Пока ПечатнаяФорма.ПроверитьВывод(Строки) Цикл
			...
			Для Каждого Эл Из МногострочныеЧасти Цикл
				Если Эл.Значение.Количество() > 0 Тогда
					...
					ОсталосьСтрокКВыводуНаПечать = ОсталосьСтрокКВыводуНаПечать - 1;//
				КонецЕсли;
			КонецЦикла;
		...
		Для Сч = 0 По СписокВыводимыхНаПечатьОбластей.Количество() - 1 Цикл
			...
			КонецЕсли;
			Если ОсобыйСлучай = 1 Тогда
				Если СписокВыводимыхНаПечатьОбластей.Получить(Сч).Представление = "ОсновнаяЧасть_1" И СписокПечатныхФорм.Количество() > 0 Тогда
				//не выводим заголовок отчета дальше первой страницы	
				ИначеЕсли СписокВыводимыхНаПечатьОбластей.Получить(Сч).Представление = "ОсновнаяЧасть_3" И ОсталосьСтрокКВыводуНаПечать > 0 Тогда
				// не выводим подвал таблицы нигде. кроме последней страницы	
				ИначеЕсли СписокВыводимыхНаПечатьОбластей.Получить(Сч).Представление = "ПустаяСтрока" Тогда
					
				ИначеЕсли СписокВыводимыхНаПечатьОбластей.Получить(Сч).Представление = "Ограничители" Тогда
		...
		ВыводимыеОбласти = Новый Массив;
		Если ОсобыйСлучай = 1 Тогда    //
			Если СписокПечатныхФорм.Количество() = 2 Тогда //
				ВыводимыеОбласти.Добавить(ОбластьОграничители); //
			КонецЕсли; //
		Иначе //
			ВыводимыеОбласти.Добавить(ОбластьОграничители);
		...
		Если флОбластьОграничителиУмещаетсяНаСтраницу Тогда
			...
			Пока флОбластьОграничителиУмещаетсяНаСтраницу Цикл
				...
				//+Иначе
					Если ОсобыйСлучай = 1 Тогда//
						Если ОсталосьСтрокКВыводуНаПечать = 0 Тогда //
...
Функция роОчистить(Форма, ЗадаватьВопросПередОчисткой = Истина, Знач ТекстВопроса = "") Экспорт
	...
	Для Каждого СтрокаУровня1 Из Форма.мСвойстваРазделовДекларации.Строки Цикл
		...
		Если СтрокаУровня1.ОчищатьРаздел Тогда
...
Функция роПоказательВходитВМногострочнуюЧасть(Форма, ИмяПоказателя) Экспорт
	...
	ИмяГруппы = Лев(ИмяПоказателя, СтрДлина(ИмяПоказателя)-2);
...
Функция роПолучитьСмысловуюЧастьИмениЯчейки(ИмяЯчейки, Определение = Истина) Экспорт
	...
	Для Инд = 1 По СтрДлина(ИмяЯчейки) Цикл
		...
		Если НЕ ТолькоЦифрыВСтроке(СимволСтроки) Тогда
...
Процедура роПриИзмененииВидаДокументаИКорректировки(Форма, РазделДокумента = Неопределено, ВидДок = Неопределено, НомКоррект = Неопределено) Экспорт
	...
	Если РазделДокумента <> Неопределено Тогда
		...
		Если ВидДок <> Неопределено Тогда
			РазделДокумента.Области[ВидДок].Значение = ?(ВыбранКорректирующийВидДок, "2", "1");
		КонецЕсли;
...
Функция роЗаполнитьСведенияОбОрганизации(Форма, Знач СписокСведений, Дополнения = "", РазделДляВывода = "Титульный") Экспорт
	...
	Если (Форма.Организация  = Неопределено) Или (Форма.Организация = ПустоеЗначениеТипа("СправочникСсылка.Организации")) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	ОргСведения = ПолучитьСведенияОбОрганизации(Форма.Организация, Форма.ДатаПодписи, СписокСведений, Форма.мДатаНачалаПериодаОтчета, Форма.мДатаКонцаПериодаОтчета);
	...
	Для Каждого ПолученныйРезультат Из ОргСведения Цикл
		...
		Если Параметр = "ИННРук" Тогда
			роПроставитьВКвадратыЗначения(Форма, "Титульный", "ИННРук", 9, ?(ПБОЮЛ, "            ", Результат));
		ИначеЕсли Параметр = "ИННБух" Тогда
			роПроставитьВКвадратыЗначения(Форма, "Титульный", "ИННГБ" , 9, ?(ПБОЮЛ, "            ", Результат));
		ИначеЕсли Параметр = "Микрофирма" Тогда
			ПроставитьЗначениеРазмераПредприятия(Форма, "Титульный", "Микрофирма" ,Результат);
		ИначеЕсли Параметр = "Численность" Тогда
			роПроставитьВКвадратыЗначения(Форма, "Титульный", "ЧисленностьСотрудников" , 5, Формат(Результат,"ЧГ="));
		ИначеЕсли Параметр = "ЧисленностьНа" Тогда
			роПроставитьВКвадратыЗначения(Форма, "Титульный", "ЧисленностьСотрудников" , 5, Формат(Результат,"ЧГ="));
		ИначеЕсли Параметр = "Площадь" Тогда
			роПроставитьВКвадратыЗначения(Форма, "Титульный", "Площадь" , 7, Формат(Результат, "ЧЦ=6; ЧДЦ=4"), Истина);
		ИначеЕсли Параметр = "ИННУпПред" Тогда
			роПроставитьВКвадратыЗначения(Форма, "Титульный", "ИННУП" , 9, ?(ПБОЮЛ, "            ", Результат));
		...
		ИначеЕсли Параметр = "ТелОрганизации" Тогда
			роПроставитьВКвадратыЗначения(Форма, "Титульный", "КодГорода", 5, НалоговаяОтчетность.КодТелефонаДляНалоговойОтчетности(Форма.Организация));
			роПроставитьВКвадратыЗначения(Форма, "Титульный", "Телефон"	 , 7, НалоговаяОтчетность.НомерТелефонаДляНалоговойОтчетности(Форма.Организация));
		ИначеЕсли Параметр = "ИННЮЛ" Тогда
			Если НЕ ПустоеЗначение(Результат) Тогда
				Результат = ДополнитьСтроку(Результат, 9, "0");
			Иначе
		...
		ИначеЕсли Параметр = "НаимОППол" Тогда
			...
			КонецЕсли;
		ИначеЕсли Параметр = "АдрПочт" Тогда
			мТитульный.Области.АдресПочтовый.Значение = Результат;
		ИначеЕсли Параметр = "АдрЮР" Тогда
			мТитульный.Области.АдресЮридический.Значение = Результат;
		ИначеЕсли Параметр = "ДатаРегистрации" Тогда
			Если НЕ ПустоеЗначение(Результат) Тогда
				Результат = Формат(Результат,"ДФ=ддММгггг");
				Результат = ДополнитьСтроку(Прав(Результат,8), 8, ".");
			Иначе
				Результат = ДополнитьСтроку(Прав(Результат,8), 8, " ");
			КонецЕсли;
			роПроставитьВКвадратыЗначения(Форма, РазделДляВывода, "ГосРег", 8, Результат)
		...
		ИначеЕсли Параметр = "КодОКОНХ" Тогда
			РегламентированнаяОтчетность.роПроставитьВКвадратыЗначения(Форма, "Титульный", "КодОКОНХ", 5, Результат);
		ИначеЕсли Параметр = "КодОКЭД" Тогда
			РегламентированнаяОтчетность.роПроставитьВКвадратыЗначения(Форма, "Титульный", "КодОКЭД", 5, Результат);
		ИначеЕсли Параметр = "СвидНО" Тогда
			роПроставитьВКвадратыЗначения(Форма, "Титульный", "СвидетельствоОГосРегистрации", 7, Результат);
		ИначеЕсли Параметр = "ОргИМНС" Тогда
			мТитульный.Области.ОргИМНС.Значение = Результат;
		КонецЕсли;
	...
	Если мТитульный.Области.Найти("ВидПредприятия1") <> Неопределено Тогда

		СтатусОрганизации=Форма.Организация.СтатусОрганизации;
		
		Если СтатусОрганизации=Перечисления.СтатусОрганизации.Микрофирма Тогда
			мТитульный.Области.ВидПредприятия1.Значение="V";
		ИначеЕсли СтатусОрганизации=Перечисления.СтатусОрганизации.МалоеПредприятие Тогда
			мТитульный.Области.ВидПредприятия2.Значение="V";
		ИначеЕсли СтатусОрганизации=Перечисления.СтатусОрганизации.КрупноеПредприятие Тогда
			мТитульный.Области.ВидПредприятия3.Значение="V";
		КонецЕсли;
		
	КонецЕсли;	
	
	Если мТитульный.Области.Найти("СвидетельствоОГосРегистрации1") <> Неопределено Тогда

		НомерСвидетельстваОГосРегистрации=Форма.Организация.НомерСвидетельстваОГосРегистрации;
		роПроставитьВКвадратыЗначения(Форма, "Титульный", "СвидетельствоОГосРегистрации", 7, НомерСвидетельстваОГосРегистрации);
...
Процедура роОчиститьВариантыЗаполнения(Форма, Знач Раздел = Неопределено, УстановитьЦветВРазделе = Истина) Экспорт
	...
	мОписаниеТиповЧислоЕ = ПолучитьОписаниеТиповЧисла(14, 2);
	ИзмененияЕсть = Ложь;
	Для Каждого СтрокаУровня1 Из Форма.мСвойстваРазделовДекларации.Строки Цикл
		// Считываем название раздела и его свойство в системе автозаполнения
		НазваниеСтраницы = СтрокаУровня1.ИмяСтраницы;
...
Процедура ВвестиКодОГРННаТитульномЛистеРегламентированногоОтчета(Форма, ОбластьИмя) Экспорт
	...
	Если Найти(ОбластьИмя, "ОГРН") > 0 Тогда
		...
		Если ВвестиСтроку(ОГРН, "Введите ОГРН " + ?(ПБОЮЛ, "предпринимателя", "организации"), ?(ПБОЮЛ, 15, 13)) Тогда
			Если (ОГРН = "") Или (ТолькоЦифрыВСтроке(ОГРН)) И (СтрДлина(ОГРН) = ?(ПБОЮЛ, 15, 13)) Тогда
...
Функция ИмпортДанныхРегламентированнойОтчетностиИзКонтейнераXML(ИмяФайла) Экспорт
	...
	ТаблицаРаздела.Колонки.Добавить("Раздел", ПолучитьОписаниеТиповСтроки(100));
...
Функция ЗаменитьТекстРегионаНаКодРегиона(Знач АдресВФормате9зпт) Экспорт
	...
	Если НЕ ТолькоЦифрыВСтроке(Регион) Тогда
...
Функция ПолучитьНазваниеРегионаПоКоду(КодРег)
	...
	Если Выборка.Количество() > 0 Тогда
		...
		Выборка.Следующий();
		Возврат ПолучитьПолноеНазвание(Выборка.Код);
...
Процедура ВыбратьИУстановитьТерриториальныйОрганФНС(Форма, СтандартнаяОбработка, ИмяТитульногоЛиста) Экспорт
	...
	ЗагрузитьВТаблицуЗначений(СправочникИФНС, ТаблИнспекций);
		...
		Если ТипЗнч(ПервоначальноеЗначение) = Тип("Строка") Тогда
			...
			Пока Вводить Цикл
				...
				Если ПроверкаВвода Тогда
					Если (ПервоначальноеЗначение = "") Или (ТолькоЦифрыВСтроке(ПервоначальноеЗначение)) И (СтрДлина(ПервоначальноеЗначение) = КритичнаяДлина) Тогда
						Вводить = Ложь;
					Иначе
				...
				Иначе
					...
					Если СтруктураТребованийРучногоВвода <> Неопределено Тогда
						...
						Если (СтруктураТребованийРучногоВвода.СтрокаТолькоИзЧисел) И (НЕ ТолькоЦифрыВСтроке(ПервоначальноеЗначение)) Тогда
		...
		Для Каждого Строка Из ТаблицаВариантыЗаполнения Цикл
			...
			Если КолвоСтрокВМногострочнойЧасти > 0 Тогда
				...
				ИначеЕсли ТипЗнч(ОписанияБлоков) = Тип("ТаблицаЗначений") Тогда
					...
					ИмяГруппы = Сред(ИмяПоказателя, 1, СтрДлина(ИмяПоказателя)-2);
...
Процедура ПроставитьИННКППНаВсехЛистахРегОтчета(Форма, НачальноеЗаполнение = Истина, Знач ИННЮЛ = Неопределено, Знач КППЮЛ = Неопределено) Экспорт
	...
	Если НЕ ПустоеЗначение(РезультатИНН) Тогда
		РезультатИНН = ДополнитьСтроку(РезультатИНН, 9, "0");
	Иначе
		РезультатИНН = ДополнитьСтроку(РезультатИНН, 9, " ");
	КонецЕсли;
	...
	Если НачальноеЗаполнение Тогда
		Для каждого Страница Из Форма.мСвойстваРазделовДекларации.Строки Цикл
			...
			Для Инд = 1 По КолвоСтраниц Цикл
				Стр = Строка(Инд) + "_";
				роПроставитьВКвадратыЗначения(Форма, ИмяСтраницы, "ИНН" + Стр, 9, РезультатИНН);
...
Процедура СформироватьСоставКнопкиОчиститьДляРегОтчетов(Форма) Экспорт
	...
	СтрокаРазделаДекларации = Форма.мСвойстваРазделовДекларации.Строки.Найти(ИмяСтраницы);
	Если СтрокаРазделаДекларации = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если СтрокаРазделаДекларации.ОчищатьРаздел Тогда
...
Функция ПолучитьВариантЗаполненияПоказателяРегОтчета(МакетСоставаПоказателей, Ном) Экспорт
	...
	Если ПозицияРазделителя > 0 Тогда
		...
		Если ТекКонфигурация = "БП" Тогда
			Возврат ВариантЗаполненияБП;
		ИначеЕсли ТекКонфигурация = "УПП" Тогда
			Возврат ВариантЗаполненияУПП;
		ИначеЕсли ТекКонфигурация = "ЗУП" Тогда
...
Функция ПолучитьСведенияОПредставителе(Форма, Знач МожетБытьТолькоЮРЛицо = Неопределено, Знач ИФНС = Неопределено) Экспорт
	...
	Если ТипЗнч(Представитель) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
		...
		ДанныеКонтактнойИнформации.Вставить("ФИОУпПред", "");
		ДанныеФЛ = РегистрыСведений.ФИОФизЛиц.СрезПоследних(Форма.ДатаПодписи, Новый Структура("ФизЛицо", Представитель));
		Если ДанныеФЛ.Количество() > 0 Тогда
			ЗначениеФИО = СокрЛП(СокрЛП(ДанныеФЛ[0].Фамилия) + " " + СокрЛП(ДанныеФЛ[0].Имя) + " " + СокрЛП(ДанныеФЛ[0].Отчество));
			ДанныеКонтактнойИнформации.Вставить("ФИОУпПред", ЗначениеФИО);
		КонецЕсли;
	...
	Если ТипЗнч(Представитель) = Тип("СправочникСсылка.Контрагенты") Тогда
		// В случае, если представитель ЮР. лицо, а в отчете только может быть Физ. лицо, тогда возвратим пустую структуру данных.
		Если МожетБытьТолькоЮРЛицо = Ложь Тогда
			Возврат ДанныеКонтактнойИнформации;
		КонецЕсли;
...
Процедура ПроверитьНапоминанияПользователяКалендарьБухгалтераСобытия(ВыбПользователь) Экспорт
	...
	Если КолВоДней = 0 Тогда
		Возврат;
	КонецЕсли;
	ТекРабочаяДата = ПолучитьРабочуюДату();
...
Функция ПроверкаРегистрацииПриОткрытииОсновнойФормыРегламентированногоОтчета(Объект)
	...
	Если НайденныйЭлемент.ВнешнийОтчетХранилище.Получить() <> Неопределено Тогда
		...
		КонецПопытки;
	Иначе
		ТекстВопроса = "Зарегистрировать выбранный внешний регламентированный отчет в информационной базе?";
	КонецЕсли;
...
Процедура ПередОткрытиемОсновнойФормыРегламентиованногоОтчета(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	КлючеваяФраза = "Внешний отчет объект";
	ЭтоВнешняяОбработка = (Лев(СтроковоеПредставлениеТипаОбъекта, СтрДлина(КлючеваяФраза)) = КлючеваяФраза);
...
//			Ложь - в противном случае.
//
Функция СуществуетДокументСАналогичнымиРеквизитами(Форма, КодИФНС,МестоНахождение=Неопределено,Плательщик=Неопределено)
	...
	Если МестоНахождение <> Неопределено Тогда
		Запрос.Текст = Запрос.Текст + "
	               |	И РегламентированныйОтчет.МестоНахождение = &МестоНахождение";
	КонецЕсли;
	
	Если Плательщик <> Неопределено Тогда
		Запрос.Текст = Запрос.Текст + "
	               |	И РегламентированныйОтчет.Плательщик = &Плательщик";
	КонецЕсли;
...
Функция ПриЗаписиРегламентированногоОтчета(Форма, КодИФНС = "не применимо", Автосохранение = Ложь, Вариант = Неопределено, МестоНахождение= Неопределено, Плательщик = Неопределено) Экспорт
	...
	Если Форма.мЗаписьЗапрещена = Истина Тогда
		...
		Возврат Ложь;
	КонецЕсли;
	Если БылиИзмененыКлючевыеРеквизитыОтчета(Форма, КодИФНС) И СуществуетДокументСАналогичнымиРеквизитами(Форма, КодИФНС, МестоНахождение,Плательщик ) Тогда
...
Процедура ПриЗакрытииРегламентированногоОтчета(Форма) Экспорт
	...
	Если Форма.мСохраненныйДок <> Неопределено И Форма.мСохраненныйДок <> ПустоеЗначениеТипа(ТипЗнч(Форма.мСохраненныйДок)) И Форма.мСохраненныйДок.Заблокирован() Тогда
		Форма.мСохраненныйДок.Разблокировать();
	КонецЕсли;
...
Процедура _ДобавитьСтрокуРасшифровки(Форма, ИмяЯчейки, ИмяПоказателя, ЗнакОперации, ИмяСлагаемого, Сумма, Счет = Неопределено, КорСчет = Неопределено, Сдвиг = Неопределено, ПраваяГраница = Неопределено, ИмяРаздела = Неопределено, НомерЛиста = Неопределено, МнЧ = Неопределено, НомерСтроки = Неопределено, Субконто1 = Неопределено, Субконто2 = Неопределено, Субконто3 = Неопределено) Экспорт
	...
	Если Сдвиг = Неопределено ИЛИ Сдвиг = ПустоеЗначениеТипа(Тип("Дата")) Тогда
...
//
Функция СформироватьТекстВыгрузки(Форма, ВремФайл)
...
Процедура ВыборФормыРегламентированногоОтчетаПоУмолчанию(Форма) Экспорт
	...
	Для Каждого Стр Из ТаблицаФормОтчета Цикл
		Если Стр.ДатаКонецДействия = ПустоеЗначениеТипа(Тип("Дата")) Тогда
...
Функция ПолучитьСведенияОбКонтрагенте(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	Если Организация = Неопределено ИЛИ Организация = ПустоеЗначениеТипа("СправочникСсылка.Контрагенты") Тогда
		Возврат ОргСведения;
	КонецЕсли;
...
Процедура ДобавитьКнопкиПечатиМашиночитаемыхФорм(Форма, Действие) Экспорт
	...
	//Если КомпонентаПечатиМашиночитаемыхФормДоступна() Тогда
		...
		//							"Печатать машиночитаемый бланк", Новый Действие("КоманднаяПанельДействийПечатьМашиночитаемогоБланка"));
		ЭлементМеню.Кнопки.Добавить("ПоказатьМашиночитаемыйБланк", ТипКнопкиКоманднойПанели.Действие,
		                            "Показать машиночитаемый бланк", Действие);
		ЭлементМеню.Кнопки.Добавить("РазделительПечать", ТипКнопкиКоманднойПанели.Разделитель);
	//КонецЕсли;
	
КонецПроцедуры

Функция ПолучитьЧисленностьСотрудниковОрганизации(Организация, НачДатаЗначения,КонДатаЗначения) Экспорт
	
	Результат = 0;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВложенныйЗапрос.Сотрудник) КАК Сотрудник
	               |ИЗ
	               |	(ВЫБРАТЬ
	               ...
	               |		И РаботникиОрганизаций.ПричинаИзмененияСостояния = ЗНАЧЕНИЕ(Перечисление.ПричиныИзмененияСостояния.Увольнение)) КАК ВложенныйЗапрос";
	
	Запрос.УстановитьПараметр("Организация", Организация);
//	Запрос.УстановитьПараметр("ПустаяДолжность", Справочники.ДолжностиОрганизаций.ПустаяСсылка());
	Запрос.УстановитьПараметр("НачалоПериода", НачалоМесяца(НачДатаЗначения));
	Запрос.УстановитьПараметр("КонецПериода", КонецМесяца(КонДатаЗначения));
...
Процедура ПроставитьЗначениеРазмераПредприятия(Форма, Лист, Показатель ,Результат)
	
	Раздел = Форма.ЭлементыФормы["ПолеТабличногоДокумента" + СтрЗаменить(Лист, "ПолеТабличногоДокумента", "")];
	
	Если Раздел.Области.Найти(Показатель) = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Результат = Перечисления.РазмерПредприятия.Микрофирма Тогда
		Раздел.Области[Показатель + ""].Значение = "V";
		Раздел.Области[Показатель + "1"].Значение = "";
		Раздел.Области[Показатель + "2"].Значение = "";
	ИначеЕсли Результат = Перечисления.РазмерПредприятия.МалоеПредприятие Тогда
		Раздел.Области[Показатель + ""].Значение = "";
		Раздел.Области[Показатель + "1"].Значение = "V";
		Раздел.Области[Показатель + "2"].Значение = "";
	ИначеЕсли Результат = Перечисления.РазмерПредприятия.КрупноеПредприятие Тогда
		Раздел.Области[Показатель + ""].Значение = "";
		Раздел.Области[Показатель + "1"].Значение = "";
		Раздел.Области[Показатель + "2"].Значение = "V";
	КонецЕсли;
...
Функция ПолучитьДанныеОтчета(Организация,ИсточникОтчета,НачалоПериода,КонецПериода,МестоНахождение=Неопределено,Плательщик=Неопределено)  Экспорт
	
	ОтборПоВидуОтчета = Новый Структура("ИсточникОтчета",ИсточникОтчета);
	
	ВыборкаДокументов = Документы.РегламентированныйОтчет.Выбрать(НачалоПериода,КонецПериода,ОтборПоВидуОтчета);
	
	ДанныеОтчета=Неопределено;
	
	Пока ВыборкаДокументов.Следующий() Цикл
		
		Если ВыборкаДокументов.Организация<>Организация Тогда
			Продолжить;
		КонецЕсли;	
		...
		Если ЗначениеЗаполнено(МестоНахождение) Тогда
			Если ВыборкаДокументов.МестоНахождение<>МестоНахождение Тогда 
				Продолжить;
			КонецЕсли;
		КонецЕсли;	
		
		Если ЗначениеЗаполнено(Плательщик) Тогда
			Если ВыборкаДокументов.Плательщик<>Плательщик  Тогда 
				Продолжить;
			КонецЕсли;
		КонецЕсли;	
		
		ДанныеОтчета = ВыборкаДокументов.ДанныеОтчета.Получить();
...
Функция ПолучитьДанныеОтчетаЗаПредыдущийПериод(Организация,ИсточникОтчета,Периодичность,ДатаОтчета,МестоНахождение=Неопределено,Плательщик=Неопределено)  Экспорт
	
	ДанныеОтчета=Неопределено;
	
	Если Периодичность = Перечисления.Периодичность.Квартал Тогда 
		КонецПрПериода = КонецКвартала(ДобавитьМесяц(ДатаОтчета, -3));
		НачалоПрПериода = НачалоКвартала(КонецПрПериода);
	Иначе
		КонецПрПериода  = КонецМесяца(ДобавитьМесяц(ДатаОтчета, -1)); 
		НачалоПрПериода = НачалоМесяца(КонецПрПериода);
	КонецЕсли;
	
	Если НачалоГода(КонецПрПериода)<>НачалоГода(ДатаОтчета) Тогда
		Возврат Неопределено;
	Иначе
		Возврат РегламентированнаяОтчетность.ПолучитьДанныеОтчета(Организация,ИсточникОтчета,НачалоПрПериода,КонецПрПериода,МестоНахождение ,Плательщик);
	КонецЕсли;	
