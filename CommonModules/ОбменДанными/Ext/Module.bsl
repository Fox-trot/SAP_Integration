...
Процедура ДобавитьИнформациюВПротокол(Знач ИнформацияДляПользователя, ДанныеПротокола = "", 
	...
	Если Не ПустаяСтрока(ИнформацияДляПользователя) Тогда
		...
		Если ЗначениеНеЗаполнено(ПараметрыСеанса.ТекущийПользователь) Тогда
			ИмяПользователя = "";
		Иначе
...
// функция возвращает строку протокола обмена данными
Функция ПолучитьПротоколОбменаПоНастройке(НастройкаОбмена)
	
	Запрос = Новый Запрос();
	Запрос.Текст = "ВЫБРАТЬ
	               |	ПараметрыОбменаДанными.ПротоколОбмена
				   |ИЗ
...
Процедура ЗаписатьДанныеПротокола(НастройкаОбмена, ДанныеПротокола)
	...
	Если НастройкаОбмена.ВестиЛог Тогда
		...
		НаборЗаписейПараметров.Отбор.НастройкаОбменаДанными.Значение = НастройкаОбмена.Ссылка;	
		...
		СтрокаПараметров = НаборЗаписейПараметров[0];
		
		Если СтрокаПараметров.ПротоколОбмена <> Неопределено Тогда
			ТекущийПротокол = СтрокаПараметров.ПротоколОбмена.Получить();
			
			Если ТекущийПротокол = Неопределено Тогда
				ТекущийПротокол = "";
			КонецЕсли;
		...
		ТекущийПротокол = ТекущийПротокол + Символы.ПС + ДанныеПротокола;
		
		НовоеЗначениеПротокола = Новый ХранилищеЗначения(ТекущийПротокол);
		
		СтрокаПараметров.ПротоколОбмена = НовоеЗначениеПротокола;
		
		Попытка
			НаборЗаписейПараметров.Записать();
		Исключение
			#Если Клиент Тогда
				Сообщить("Ошибка при сохранении протокола обмена данными для настройки  """ + НастройкаОбмена.Ссылка + """ : " + ОписаниеОшибки(), СтатусСообщения.Важное);
			#КонецЕсли
		КонецПопытки;
...
// процедура очищает протокол обмена настройки
Процедура ОчиститьПротоколОбмена(НастройкаОбмена) Экспорт
	...
	#Если Клиент Тогда
		ОтветПользователя = Вопрос("Вы уверены в том, что хотите очистить протокол обмена данными?", РежимДиалогаВопрос.ДаНет, 30,
			КодВозвратаДиалога.Нет, "Очистка протокола");
	...
	Если ТипЗнч(НастройкаОбмена) = Тип("СправочникСсылка.НастройкиОбменаДанными") Тогда
		СсылкаНаНастройку = НастройкаОбмена;
	Иначе
		СсылкаНаНастройку = НастройкаОбмена.Ссылка;	
	КонецЕсли;

	// надо проверить наличие одной записи в регистре сведений
	НаборЗаписейПараметров = РегистрыСведений.ПараметрыОбменаДанными.СоздатьНаборЗаписей();
		
	НаборЗаписейПараметров.Отбор.НастройкаОбменаДанными.Значение = СсылкаНаНастройку;	
	НаборЗаписейПараметров.Отбор.НастройкаОбменаДанными.Использование = Истина;
	НаборЗаписейПараметров.Отбор.НастройкаОбменаДанными.ВидСравнения = ВидСравнения.Равно;
	...
	СтрокаПараметров.ПротоколОбмена = Неопределено;
	
	Попытка
		НаборЗаписейПараметров.Записать();
	Исключение
		#Если Клиент Тогда
			Сообщить("Ошибка при очистке протокола обмена данными для настройки  """ + НастройкаОбмена.Ссылка + """ : " + ОписаниеОшибки(), СтатусСообщения.Важное);
		#КонецЕсли
	КонецПопытки;
...
// процедура открывает файл лога настройки обмена
Процедура ПоказатьФайлЛогаНастройкиОбмена(НастройкаОбмена, Владелец = Неопределено) Экспорт
	...
	// получаем строку протокола из регистра
	Протокол = ПолучитьПротоколОбменаПоНастройке(СсылкаНаНастройку);
	
	// показываем форму
	ФормаПротокола = ПолучитьОбщуюФорму("ФормаРедактированияТекста", Владелец);
	ФормаПротокола.Заголовок = "Протокол обмена данными";
	ФормаПротокола.УстановитьРедактируемыйТекст(Протокол);
	ФормаПротокола.ЭлементыФормы.ПолеТекстовогоДокументаДляРедактирования.ТолькоПросмотр = Истина;
	
	ФормаПротокола.Открыть();
...
Функция ПолучитьИмяПланаОбмена(УзелОбмена) Экспорт
	...
	Если ЗначениеНеЗаполнено(УзелОбмена) Тогда
		Возврат "";
	КонецЕсли;
...
Функция ПолучитьТекущийУзелИБ(УзелОбмена) Экспорт
	...
	Если ЗначениеНеЗаполнено(ИмяПланаОбмена) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Если СтруктураНастроекОбменаДанными.ПроизводитьЧтениеДанных Тогда
		...
		Иначе
			...
			Если (МассивНайденныхФайлов.Количество() > 0)
				И НЕ ЗначениеНеЗаполнено(СтруктураНастроекОбменаДанными.ДатаПоследнегоФайлаОбмена) Тогда
...
Процедура ЗафиксироватьЗавершениеОбмена(СтруктураДанныхНастройкиОбмена, Знач СтрокаСообщенияОбОшибке = "", Знач НеВыводитьИнформациюПользователю = Ложь)
	...
	Если Не ЗначениеНеЗаполнено(СтруктураДанныхНастройкиОбмена.ДатаПоследнейЗагрузки)
	...
	Если Не ЗначениеНеЗаполнено(СтруктураДанныхНастройкиОбмена.ДатаПоследнейВыгрузки)
	...
	ЗаписатьДанныеПротокола(СтруктураДанныхНастройкиОбмена.ДанныеНастройки, СтруктураДанныхНастройкиОбмена.ДанныеПротокола);
...
Функция ВыполнитьДействияПередНачаломОбменаДанными(СтруктураНастроекОбменаДанными)
	...
	Если ЗначениеНеЗаполнено(СтруктураНастроекОбменаДанными.ДанныеНастройки.УзелИнформационнойБазы) Тогда
