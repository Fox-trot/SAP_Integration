...
Функция ПолучитьСтруктуруПоказателейОтчета(БУ = Истина, ВалютнаяСумма = Ложь, Количество = Ложь, РазвернутоеСальдо = Ложь) Экспорт
...
Функция ПолучитьТекстЗаголовка(ОтчетОбъект, ЗаголовокТекст = "", ОрганизацияВНачале = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(ОтчетОбъект.Организация) Тогда
		СведенияОбОрганизации    = ОтчетОбъект.Организация.НаименованиеПолное;//СведенияОЮрФизЛице(ОтчетОбъект.Организация);
		ТекстОрганизации = ОтчетОбъект.Организация.НаименованиеПолное;//ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "НаименованиеДляПечатныхФорм,");
...
Функция ПолучитьТекстОрганизация(ОтчетОбъект = Неопределено, Организация = Неопределено, ВключатьОбособленныеПодразделения = Ложь) Экспорт
	...
	Попытка
		...
		//	КонецЕсли;
		//КонецЕсли;
		ТекстОрганизации = ОтчетОбъект.Организация.НаименованиеПолное;
	Исключение
	КонецПопытки;
	...
	Для каждого Тип Из ТипЗначения.Типы() Цикл
		...
		Если Метаданные.Справочники.Содержит(ОбъектМетаданных) Тогда
			Если ОбъектМетаданных.Иерархический Тогда
				Возврат ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь, "ТипДетализацииСтандартныхОтчетов");
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьТипИзмеренияПостроителяОтчетаПоУмолчанию()
	...
	ТипДетализацииПоУмолчанию = ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь, "ТипДетализацииСтандартныхОтчетов");
...
Процедура СохранитьНастройку(ОтчетОбъект) Экспорт
	...
	Если ОтчетОбъект.Метаданные().Имя="ОборотноСальдоваяВедомость" Тогда
		Попытка
			СтруктураНастроек.ДанныеОтчета.Удалить("МакетШапкиОтчета");
		Исключение
		КонецПопытки;
		Попытка
			СтруктураНастроек.ДанныеОтчета.Удалить("МакетСчет");
		Исключение
		КонецПопытки;
		Попытка
			СтруктураНастроек.ДанныеОтчета.Удалить("МакетПодвал");
		Исключение
		КонецПопытки;
	КонецЕсли;
...
Процедура ЗаполнитьДополнительныеПоляПоУмолчанию(ОтчетОбъект) Экспорт
	...
	Для Каждого Группировка Из ДанныеОтчета.Группировка Цикл 
		...
		//	НоваяСтрока.Представление = СтандартныеОтчеты.ПолучитьСвойствоПоля(КомпоновщикНастроек, НоваяСтрока.Поле);
		ИначеЕсли ТипПоля.СодержитТип(Тип("СправочникСсылка.Номенклатура")) Тогда
			НоваяСтрока = ДанныеОтчета.ДополнительныеПоля.Добавить();
			НоваяСтрока.Использование = Ложь;
			НоваяСтрока.Поле          = Строка(Группировка.Поле) + ".Код";
			НоваяСтрока.Представление = СтандартныеОтчеты.ПолучитьСвойствоПоля(КомпоновщикНастроек, НоваяСтрока.Поле);	
			
			НоваяСтрока = ДанныеОтчета.ДополнительныеПоля.Добавить();
			НоваяСтрока.Использование = Ложь;
			НоваяСтрока.Поле          = Строка(Группировка.Поле) + ".Цена";
			НоваяСтрока.Представление = СтандартныеОтчеты.ПолучитьСвойствоПоля(КомпоновщикНастроек, НоваяСтрока.Поле);	
			
			НоваяСтрока = ДанныеОтчета.ДополнительныеПоля.Добавить();
			НоваяСтрока.Использование = Ложь;
			НоваяСтрока.Поле          = Строка(Группировка.Поле) + ".БазоваяЕдиницаИзмерения";
			НоваяСтрока.Представление = СтандартныеОтчеты.ПолучитьСвойствоПоля(КомпоновщикНастроек, НоваяСтрока.Поле);	
		ИначеЕсли ТипПоля.СодержитТип(Тип("СправочникСсылка.ОсновныеСредства")) Тогда
			...
			НоваяСтрока = ДанныеОтчета.ДополнительныеПоля.Добавить();
			НоваяСтрока.Использование = Ложь;
			НоваяСтрока.Поле          = Строка(Группировка.Поле) + ".ИнвентарныйНомер";
			НоваяСтрока.Представление = СтандартныеОтчеты.ПолучитьСвойствоПоля(КомпоновщикНастроек, НоваяСтрока.Поле);	
		//ИначеЕсли ТипПоля.СодержитТип(Тип("СправочникСсылка.ОсновныеСредства")) Тогда
		//	НоваяСтрока = ДанныеОтчета.ДополнительныеПоля.Добавить();
		//	НоваяСтрока.Использование = Ложь;
...
Процедура ЗаполнитьДанныеОтчета(ОтчетОбъект) Экспорт
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.АнализСчета" Тогда
		Если Не ОтчетОбъект.ДанныеОтчета.Свойство("ГруппировкаКор") Тогда
			...
			ГруппировкаКор.Колонки.Добавить("Представление", ПолучитьОписаниеТиповСтроки(200));
			МассивТипов = Новый Массив;
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.ОборотноСальдоваяВедомость" Тогда
		Если Не ОтчетОбъект.ДанныеОтчета.Свойство("Группировка") Тогда
			...
			Группировка.Колонки.Добавить("Представление", ПолучитьОписаниеТиповСтроки(200));
			МассивТипов = Новый Массив;
			МассивТипов.Добавить(Тип("СписокЗначений"));
		...
		Если Не ОтчетОбъект.ДанныеОтчета.Свойство("РазвернутоеСальдо") Тогда
			...
			РазвернутоеСальдо.Колонки.Добавить("Представление", ПолучитьОписаниеТиповСтроки(200));
			МассивТипов = Новый Массив;
			МассивТипов.Добавить(Тип("СписокЗначений"));
