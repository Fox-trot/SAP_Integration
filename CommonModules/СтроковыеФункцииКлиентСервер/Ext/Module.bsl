...
//
Функция РазложитьСтрокуВМассивПодстрокЭД(Знач Строка, Знач Разделитель = ",", Знач ПропускатьПустыеСтроки = Неопределено) Экспорт
...
Функция СтрокаИзМассиваПодстрок(Массив, Разделитель = ",") Экспорт
	Возврат ПолучитьСтрокуИзМассиваПодстрок(Массив, Разделитель);
КонецФункции
...
//
Функция ТолькоЦифрыВСтрокеЭД(Знач СтрокаПроверки, Знач УчитыватьЛидирующиеНули = Истина, Знач УчитыватьПробелы = Истина) Экспорт
...
// Удаляет незначащие нули из строки.
//
// Параметры:
//  Строка       - Строка - строка для удаления нулей.
//
Функция УдалитьНезначащиеНули(Строка) Экспорт
	СтрокаРезультат = Строка;
	Если Не ТолькоЦифрыВСтрокеЭД(СтрокаРезультат) Тогда
		Возврат СтрокаРезультат;
	КонецЕсли;
	Пока СтрДлина(СтрокаРезультат) > 1 И Лев(СтрокаРезультат, 1) = "0" Цикл
...
Функция ПолучитьПараметрыИзСтроки(Знач СтрокаПараметров) Экспорт
	...
	МассивПодстрок = РазложитьСтрокуВМассивПодстрокЭД(СтрокаПараметров, ";");
...
Функция ВерсияКонфигурацииБезНомераСборки(Знач Версия) Экспорт
	...
	Массив = РазложитьСтрокуВМассивПодстрокЭД(Версия, ".");
