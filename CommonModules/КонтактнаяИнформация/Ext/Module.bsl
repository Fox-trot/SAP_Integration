...
//
Функция СведенияОЮрФизЛице_(ЮрФизЛицо, БанковскийСчет, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина,Получатель=Ложь) Экспорт

	Сведения = Новый Структура("Наименование,Руководитель, ИНН, ОКОНХ,ОКПО,ОКЭД, Телефоны, ЮридическийАдрес, Банк, МФО, НомерСчета, АдресБанка");
	Запрос   = Новый Запрос;
	Данные   = Неопределено;
	...
	Если ИмяСправочника = "Контрагенты" Тогда
		Запрос.Текст = Запрос.Текст +",ФИОДиректораВЛице КАК Руководитель, 
		|   ДолжностьРуководителя КАК ДолжностьРуководителя,
		|	ДокументУдостоверяющийЛичность КАК ДокументУдостоверяющийЛичность,
		|	КемВыданДокументУдостоверяющийЛичность КАК КемВыданДокументУдостоверяющийЛичность,
		|	ДействителенДоДокументУдостоверяющийЛичность КАК ДействителенДоДокументУдостоверяющийЛичность";
	КонецЕсли; 
	
	Если ИмяСправочника = "Организации" Тогда
		Запрос.Текст = Запрос.Текст +",НаименованиеНаАнглийском КАК НаименованиеНаАнглийском";
	КонецЕсли; 
	...
	Если Данные <> Неопределено Тогда
		...
		Сведения.Вставить(ИмяПоля+"Наименование",    ?(сокрлп(Данные.НаименованиеПолное=""),сокрлп(Данные.наименование),сокрлп(Данные.НаименованиеПолное)))
		;
		Если ИмяСправочника="Организации" Тогда
			Сведения.Вставить(ИмяПоля+"НаименованиеНаАнглийском",сокрлп(Данные.НаименованиеНаАнглийском));
		КонецЕсли;	
		
		Сведения.Вставить(ИмяПоля+"ИНН",              сокрлп(Данные.ИНН));
		Сведения.Вставить(ИмяПоля+"ОКПО",             сокрлп(Данные.ОКПО));
		Сведения.Вставить(ИмяПоля+"ОКЭД",             сокрлп(Данные.ОКЭД));
		Сведения.Вставить(ИмяПоля+"ОКОНХ",            сокрлп(Данные.ОКОНХ));
		Сведения.Вставить(ИмяПоля+"Телефоны",         сокрлп(ПолучитьТелефонИзКонтактнойИнформации(ЮрФизЛицо)));
		Сведения.Вставить(ИмяПоля+"ЮридическийАдрес", сокрлп(ПолучитьАдресИзКонтактнойИнформации(ЮрФизЛицо, "Юридический")));
		Сведения.Вставить(ИмяПоля+"ЮридическийАдресНаАнглийском", сокрлп(ПолучитьАдресИзКонтактнойИнформации(ЮрФизЛицо, "ЮридическийНаАнглийском")));
		Сведения.Вставить(ИмяПоля+"ФактическийАдрес", сокрлп(ПолучитьАдресИзКонтактнойИнформации(ЮрФизЛицо, "Фактический")));
		
		Если ЮрФизЛицо.ЮрФизЛицо=Перечисления.ЮрФизЛицо.ФизЛицо Тогда
			Сведения.Вставить(ИмяПоля+"ДокументУдостоверяющийЛичность", сокрлп(Данные.ДокументУдостоверяющийЛичность));
			Сведения.Вставить(ИмяПоля+"КемВыданДокументУдостоверяющийЛичность", сокрлп(Данные.КемВыданДокументУдостоверяющийЛичность));
			Сведения.Вставить(ИмяПоля+"ДействителенДоДокументУдостоверяющийЛичность", сокрлп(Данные.ДействителенДоДокументУдостоверяющийЛичность));
		КонецЕсли; 
		
		Если ИмяСправочника = "Контрагенты" Тогда
			Если Сокрлп(Данные.Руководитель)="" Тогда
				Сведения.Вставить(ИмяПоля+"Руководитель", "_______________________");
			Иначе
				Сведения.Вставить(ИмяПоля+"Руководитель", Данные.Руководитель);
			КонецЕсли;
			
			Если Данные.ДолжностьРуководителя="" Тогда
				Сведения.Вставить(ИмяПоля+"ДолжностьРуководителя", "_______________________");
			Иначе
				Сведения.Вставить(ИмяПоля+"ДолжностьРуководителя", Данные.ДолжностьРуководителя);
			КонецЕсли; 
		КонецЕсли; 
	КонецЕсли;
	
	Если ЗначениеНеЗаполнено(БанковскийСчет) Тогда
		Возврат Сведения;
	КонецЕсли;
	...
	|	БС.Банк       КАК Банк,
	|	БС.Банк.НаименованиеНаАнглийском КАК БанкНаименованиеНаАнглийском,
	|	БС.Банк.Код   КАК МФО,
	|	БС.Банк.Город КАК ГородБанка,
	|	БС.Банк.Адрес КАК АдресБанка
	|ИЗ
	|	Справочник.БанковскиеСчета КАК БС
	|
	|ГДЕ
	|	БС.Ссылка = &БанковскийСчет И БС.Владелец=&ПарЮрФизЛицо";
	Данные = Запрос.Выполнить().Выбрать();
	Данные.Следующий();

	Если Данные <> Неопределено Тогда

		Сведения.Вставить(ИмяПоля+"НомерСчета",       сокрлп(Данные.НомерСчета));
		Сведения.Вставить(ИмяПоля+"Банк",             сокрлп(Данные.Банк));
		Сведения.Вставить(ИмяПоля+"БанкНаименованиеНаАнглийском",             сокрлп(Данные.БанкНаименованиеНаАнглийском));
		Сведения.Вставить(ИмяПоля+"МФО",              сокрлп(Данные.МФО));
		Сведения.Вставить(ИмяПоля+"АдресБанка",       сокрлп(Данные.АдресБанка));
		Сведения.Вставить(ИмяПоля+"ГородБанка",       сокрлп(Данные.ГородБанка));
	КонецЕсли;

	Возврат Сведения;

КонецФункции // СведенияОЮрФизЛице()
Функция СведенияОЮрФизЛице(ЮрФизЛицо, БанковскийСчет, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина,Получатель=Ложь) Экспорт
	...
	|	ИНН,
	|	КодПоОКПО КАК ОКПО,
	|	КодПлательщикаНДС КАК КодПлательщикаНДС,
	|	КодОКЭД КАК ОКЭД,
	|	КодОКОНХ КАК ОКОНХ";
	...
	Если ИмяСправочника = "Организации" Тогда
		Запрос.Текст = Запрос.Текст +",НаименованиеНаАнглийском КАК НаименованиеНаАнглийском";
		Запрос.Текст = Запрос.Текст +",НаименованиеОПФ КАК НаименованиеОПФ";
		Запрос.Текст = Запрос.Текст +",НаименованиеФС КАК НаименованиеФС";
		Запрос.Текст = Запрос.Текст +",КодСОАТО КАК КодСОАТО";
		Запрос.Текст = Запрос.Текст +",КодОКОПФ КАК КодОКОПФ";
		Запрос.Текст = Запрос.Текст +",КодОКФС КАК КодОКФС";
		Запрос.Текст = Запрос.Текст +",КодСООГУ КАК КодСООГУ";
	КонецЕсли; 
	...
	Если Данные <> Неопределено Тогда
		...
		Если ИмяСправочника="Организации" Тогда
			Сведения.Вставить(ИмяПоля+"НаименованиеНаАнглийском",сокрлп(Данные.НаименованиеНаАнглийском));
			Сведения.Вставить(ИмяПоля+"НаименованиеОПФ",сокрлп(Данные.НаименованиеОПФ));
			Сведения.Вставить(ИмяПоля+"НаименованиеФС",сокрлп(Данные.НаименованиеФС));
			Сведения.Вставить(ИмяПоля+"КодСОАТО",сокрлп(Данные.КодСОАТО));
			Сведения.Вставить(ИмяПоля+"КодОКОПФ",сокрлп(Данные.КодОКОПФ));
			Сведения.Вставить(ИмяПоля+"КодОКФС",сокрлп(Данные.КодОКФС));
			Сведения.Вставить(ИмяПоля+"КодСООГУ",сокрлп(Данные.КодСООГУ));
			Сведения.Вставить(ИмяПоля+"ПочтовыйАдрес", сокрлп(ПолучитьАдресИзКонтактнойИнформации(ЮрФизЛицо, "Почтовый")));
		КонецЕсли;	
		...
		Сведения.Вставить(ИмяПоля+"ОКЭД",             сокрлп(Данные.ОКЭД));
		Сведения.Вставить(ИмяПоля+"ОКОНХ",            сокрлп(Данные.ОКОНХ));
		Сведения.Вставить(ИмяПоля+"КодПлательщикаНДС",            сокрлп(Данные.КодПлательщикаНДС));
		Сведения.Вставить(ИмяПоля+"Телефоны",         сокрлп(ПолучитьТелефонИзКонтактнойИнформации(ЮрФизЛицо)));
		Сведения.Вставить(ИмяПоля+"ЮридическийАдрес", сокрлп(ПолучитьАдресИзКонтактнойИнформации(ЮрФизЛицо, "Юридический")));
		Сведения.Вставить(ИмяПоля+"ЮридическийАдресНаАнглийском", сокрлп(ПолучитьАдресИзКонтактнойИнформации(ЮрФизЛицо, "ЮридическийНаАнглийском")));
		Сведения.Вставить(ИмяПоля+"ФактическийАдрес", сокрлп(ПолучитьАдресИзКонтактнойИнформации(ЮрФизЛицо, "Фактический")));
		...
		Если ИмяСправочника = "Контрагенты" Тогда
			Если Сокрлп(Данные.Руководитель)="" Тогда
				Сведения.Вставить(ИмяПоля+"Руководитель", "_____________________________");
			Иначе
				Сведения.Вставить(ИмяПоля+"Руководитель", Данные.Руководитель);
			КонецЕсли; 
		КонецЕсли; 
...
Функция ПолучитьАдресИзКонтактнойИнформации(Объект, ТипАдреса = "") Экспорт
	...
	ИначеЕсли ТипАдреса="Регистрации"
		...
		Начало = "ЮрАдрес";
	ИначеЕсли ТипАдреса="ЮридическийНаАнглийском" Тогда
		Начало = "ЮрАдресНаАнглийском";
	Иначе
		Возврат "";
	КонецЕсли;
	...
	ВидАдреса = Справочники.ВидыКонтактнойИнформации[Начало+Окончание];
...
Процедура АдресныйЭлементЗаполнитьРодителей(Регион, Район, Город, НаселенныйПункт, Элемент) Экспорт
	...
	Если (Элемент.ТипАдресногоЭлемента > 4) И ЗначениеНеЗаполнено(НаселенныйПункт) Тогда
		НаселенныйПункт = ПолучитьИмяАдресногоЭлементаПоКоду(Элемент.Код - Элемент.Код%МаскаНасПункта(), 4);
	КонецЕсли;

	Если (Элемент.ТипАдресногоЭлемента > 3) И ЗначениеНеЗаполнено(Город) Тогда
		Город = ПолучитьИмяАдресногоЭлементаПоКоду(Элемент.Код - Элемент.Код%МаскаГорода(), 3);
	КонецЕсли;

	Если (Элемент.ТипАдресногоЭлемента > 2) И ЗначениеНеЗаполнено(Район) Тогда    
		Район = ПолучитьИмяАдресногоЭлементаПоКоду(Элемент.Код - Элемент.Код%МаскаРайона(), 2);
	КонецЕсли;

	Если (Элемент.ТипАдресногоЭлемента > 1) И ЗначениеНеЗаполнено(Регион) Тогда
...
Процедура АдресныйЭлементПроверитьПравильностьЗаполненияПотомков(Регион, Район, Город, НаселенныйПункт, Улица, Элемент) Экспорт
	...
	Если (Элемент.ТипАдресногоЭлемента < 2) И НЕ ЗначениеНеЗаполнено(ИмяРайона) Тогда
	...
	Если (Элемент.ТипАдресногоЭлемента < 3) И НЕ ЗначениеНеЗаполнено(ИмяГорода) Тогда
	...
	Если (Элемент.ТипАдресногоЭлемента < 4) И НЕ ЗначениеНеЗаполнено(ИмяНаселенногоПункта) Тогда
	...
	Если (Элемент.ТипАдресногоЭлемента < 5) И НЕ ЗначениеНеЗаполнено(ИмяУлицы) Тогда
...
// Функция возвращает представление адреса за пределами РУз без Страны
Функция ПолучитьПредставлениеАдресаЗаПределамиРУзБезСтраны(Страна, Представление) Экспорт
	
	Если Не ЗначениеНеЗаполнено(Страна) И Страна <> Справочники.КлассификаторСтранМира.УЗБЕКИСТАН.Наименование Тогда
...
// Процедура формирует представление для Адреса за пределами РУз
Функция СформироватьПредставлениеАдресаЗаПределамиРУз(Страна, Представление) Экспорт
