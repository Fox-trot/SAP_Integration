...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	ДанныеИзРегистраСведений = РегистрыСведений.ПриложенияКРасчетуНалогаНаПрибыль.СрезПоследних(ТекущаяДата(),новый структура("СтатьяЗатрат",Объект.Ссылка));
	Если ДанныеИзРегистраСведений.Количество()<> 0 Тогда 
	Объект.ВозвращатьВНОБДляНалогаНаПрибыль = ДанныеИзРегистраСведений[0].НеВычетаемыеРасходыВключаемыеНОБ;	
	Объект.ГрафаПриложения1 = ДанныеИзРегистраСведений[0].СтрокаПриложения3;	
	Объект.ВозвращатьВНОБДляНалогаНаПрибыль = ДанныеИзРегистраСведений[0].НеВычетаемыеРасходыВключаемыеНОБ;	
	Объект.Приложение3 = ДанныеИзРегистраСведений[0].НеВычетаемыеРасходыВключаемыеНОБОтчетногоПериода;	
	Объект.ГрафаПриложения3 = ДанныеИзРегистраСведений[0].СтрокаПриложения4;	
	Объект.Приложение5 = ДанныеИзРегистраСведений[0].УменьшениеНОБ;	
	Объект.ГрафаПриложения5 = ДанныеИзРегистраСведений[0].СтрокаПриложения7;	
	КонецЕсли;
КонецПроцедуры
