...
Процедура КомандаПоступлениеНоменклатурыСоздать(Команда)
	Если ПроверитьЗаполнение() Тогда
		...
		Ответ	= ZMM_000_FM_1C_FATURA_LIST_RTRNНаСервере();
		Если НЕ ПустаяСтрока(Ответ) Тогда
			Сообщить(Ответ);
			ZMM_000_FM_1C_FATURA_LISTESIНаСервере();
		КонецЕсли;
...
Процедура СоответсвияЗагрузитьНаСервере()
	...
	Колонка_Код				= 1;
	Колонка_Код1С			= 2;
	//Колонка_Наименование	= 3;
	Колонка_Серия			= 4;
	
	Замена = ЗаменыПолучить();
	Итератор				= 2;
	Пока Итератор <= ?(Объект.ЭтоТест, 9, ТабДок.ВысотаТаблицы) Цикл
		...
		Для Каждого ТекЭлемент Из Замена Цикл
			Серия	= СтрЗаменить(Серия, ТекЭлемент.Ключ, ТекЭлемент.Значение);
		КонецЦикла;
...
Функция НоменклатураПродажНайти(Код, Серия)
	Возврат НаСервере().НоменклатураПродажНайти2(Код, Серия);
КонецФункции
...
Процедура ZSD_011_S_DATAВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Ссылка = SDДокументНайти(Новый Структура("BILLDOC,BILLDATE,BILLTYP", Элемент.ТекущиеДанные.BILLDOC, Элемент.ТекущиеДанные.BILLDATE, Элемент.ТекущиеДанные.BILLTYP));
	Если ЗначениеЗаполнено(Ссылка) Тогда
		ПоказатьЗначение(, Ссылка);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбменСАП()
	...
	ФоновыеЗадания.Выполнить("ОбменСАП.ВыполнитьОбмен");
КонецПроцедуры

&НаСервереБезКонтекста
Процедура КомандаЗаменаКирилицыНаСервере()
	Замена = ЗаменыПолучить();
	Запрос	= Новый Запрос("ВЫБРАТЬ
	      	               ...
	      	               |ГДЕ
	      	               |	НоменклатураПродаж.ЭтоГруппа = ЛОЖЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого ТекЭлемент Из Замена Цикл
			Серия	= СтрЗаменить(Серия, ТекЭлемент.Ключ, ТекЭлемент.Значение);
		КонецЦикла;
		Если Выборка.Серия <> Серия Тогда
			Спр	= Выборка.Ссылка.ПолучитьОбъект();
			Спр.Серия	= СокрЛП(Серия);
...
&НаКлиенте
Процедура КомандаЗаменаКирилицы(Команда)
	КомандаЗаменаКирилицыНаСервере();
КонецПроцедуры
