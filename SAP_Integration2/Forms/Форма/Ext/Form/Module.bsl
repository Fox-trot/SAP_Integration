...
Функция ZFI_011_RFC_OZBEKISTAN_RTRNНаСервере()
	Возврат НаСервере().ZFI_011_RFC_OZBEKISTAN_RTRN(Объект.ZFI_011_FG_1C);
КонецФункции

&НаКлиенте
Процедура ZFI_011_RFC_OZBEKISTAN_RTRN(Команда)
	Если Объект.ZFI_011_FG_1C.Количество() > 0 Тогда
		Сообщить(ZFI_011_RFC_OZBEKISTAN_RTRNНаСервере());
	КонецЕсли;
КонецПроцедуры
...
Процедура ZFI_011_RFC_OZBEKISTAN(Команда)
	Если Элементы.ZFI_011_S_DATA.Количество() > 0 Тогда
		ZFI_011_RFC_OZBEKISTANНаСервере();
	КонецЕсли;
КонецПроцедуры

&НаСервере
Функция КомандаПодтвердитьНаСервере()
	Если НаСервере.ZFI_011_RFC_OZBEKISTAN_RTRN(Объект.ZFI_011_S_DATA) Тогда
