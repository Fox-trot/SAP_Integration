...
Процедура Логировать(Слова, Уровень=Неопределено)
	ЗаписьЖурналаРегистрации("ОбменСАП", ?(Уровень=Неопределено, УровеньЖурналаРегистрации.Ошибка, Уровень),,, Слова);
КонецПроцедуры

Функция ZFI_011_RFC_OZBEKISTAN_RTRN(ТЧ=Неопределено, DOCNO="") Экспорт
	Если НЕ ЭтоПродуктив() Тогда
		Возврат "";
	...
	Иначе
		...
		Для Каждого ТекСтрока Из мСтроки Цикл
			Если НЕ ПустаяСтрока(ТекСтрока.REFDOCNO_TOREVERSE) Или ЗначениеЗаполнено(ZFI_011_ДокументНайти(ТекСтрока)) Тогда
				ZFI_011_RFC_OZBEKISTAN_RTRN_Заполнить(WSПараметр, WSЗапись, ТекСтрока);
				Загружено.Добавить(ТекСтрока.DOCNO);
			КонецЕсли;
			Прервать;
	...
	Если Загружено.Количество() > 0 И ПустаяСтрока(Ответ) Тогда
		...
		Логировать("ZFI_011_RFC_OZBEKISTAN_RTRN " + ?(ПустаяСтрока(Ответ), СтрСоединить(Загружено, ";"), СтрСоединить(Загружено, ";") + Символы.ПС + Ответ), УровеньЖурналаРегистрации.Информация);
	КонецЕсли;
	Возврат Ответ;
КонецФункции
...
Функция Настроить(Объект=Неопределено) Экспорт
	Если Объект = Неопределено Тогда
		Объект	= ЭтотОбъект;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Или НЕ ЗначениеЗаполнено(Объект.Ответственный) Тогда
		...
		Если ЗначениеЗаполнено(Объект.Ответственный) Тогда
			УстановитьПараметрыСеанса(Объект.Ответственный);
		КонецЕсли;
...
Процедура ПоступлениеНоменклатурыВалютныйСоздать(Шапка, Детали)
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		Если НЕ ДокументПометить(Док) Тогда Возврат; КонецЕсли;
	...
	ДокЗаписать(Док, Ошибки, Шапка.SATINALMA_SIPARIS);
КонецПроцедуры
...
Функция КонтрагентНайти(Знач ИНН, Наименование="")
	...
	Запрос.УстановитьПараметр("Код",			СокрЛП(ИНН));
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Ответ = Выборка.Ссылка;
	Иначе
	...
	Для Каждого ТекСтрока Из ТЧ Цикл
		Если мДоки.Найти(ТекСтрока.BILLDOC) = Неопределено Тогда
			...
			//Иначе
				...
				Если SD_Отмена(ТекСтрока.BILLTYP) Тогда
					...
					КонецЕсли;
					ДокументПометить(Корректируемый, Истина);
...
Функция SD_Отмена(BILLTYP)
	Возврат (СтрСравнить(BILLTYP, "S1") = 0);
КонецФункции

Функция КлючевоеСловоСодержится(Слово)
	Возврат (СтрНайти(НРег(Слово), "не трогать") > 0);
КонецФункции
...
Процедура РеализацияМедикаментовСоздать(Шапка, Детали)
	...
	Для Каждого Деталь Из Детали Цикл
			...
			Если ЗначениеЗаполнено(ТекСтрока.Номенклатура) Тогда
				...
				ТекСтрока.СчетУчетаБУ	= СчетУчетаНоменклатурыНайти(ТекСтрока.Номенклатура, СчетУчетаБУ);
			Иначе
			...
			Если Док.ФинансоваяСкидкаСумма > 0 И Док.ФинансоваяСкидкаПроцент = 0 Тогда
	...
	ДокЗаписать(Док, Ошибки, Шапка.BILLDOC);
КонецПроцедуры

Функция СчетУчетаНоменклатурыНайти(Номенклатура, Знач СчетУчетаБУ)
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		Запрос	= Новый Запрос(
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	Хозрасчетный.Счет КАК Счет
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.ОстаткиИОбороты(, &ДатаН, Месяц, , Счет.Родитель.Код = ""2900"", &Аналитика, Организация = &Организация) КАК Хозрасчетный
		|
		|УПОРЯДОЧИТЬ ПО
		|	Хозрасчетный.Период УБЫВ,
		|	Хозрасчетный.КоличествоКонечныйОстатокДт УБЫВ,
		|	Хозрасчетный.КоличествоОборот УБЫВ");
		Запрос.УстановитьПараметр("ДатаН",			ТекущаяДатаСеанса());
		Аналитика = Новый Массив;
		Аналитика.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатураПродаж);
		Аналитика.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Склады);
...
Функция ДокументПометить(Виктим, Пометка=Ложь, Принудительно=Ложь)
	Если ?(СтрНайти(Строка(ТипЗнч(Виктим)), "объект") = 0, Виктим.ПометкаУдаления <> Пометка, ЗначениеЗаполнено(Виктим.Ссылка) И Виктим.ПометкаУдаления <> Пометка) Тогда
		Если СтрНайти(Строка(ТипЗнч(Виктим)), "объект") = 0 Тогда
			Док	= Виктим.ПолучитьОбъект();
		Иначе
			Док	= Виктим;
		КонецЕсли;
		Если Принудительно Или НЕ КлючевоеСловоСодержится(Док.Комментарий) Тогда
			Попытка
				Док.ОбменДанными.Загрузка	= Истина;
				Док.УстановитьПометкуУдаления(Пометка);
			Исключение
				Логировать(Строка(Виктим) + " " + ОписаниеОшибки());
				Возврат Ложь;
			КонецПопытки;
		Иначе
			Возврат Ложь;
...
Функция SDДокументНайти(Параметры) Экспорт
	...
	Запрос.УстановитьПараметр("Номер",			?(SD_Отмена(Параметры.BILLTYP), Параметры.REVREC, Параметры.BILLDOC));
	Запрос.УстановитьПараметр("Организация",	Организация);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Ответ =  Выборка.Ссылка;
...
Процедура УслугиСтороннихСоздатьНаСервере(ТЧ=Неопределено) Экспорт
	...
	Для Каждого ТекСтрока Из ТЧ Цикл
		Если мДоки.Найти(ТекСтрока.DOCNO) = Неопределено Тогда
			...
			ИначеЕсли БанкКонвертация(ТекСтрока.DOCTYP) Тогда
				КонвертацияБанкСоздать(ТекСтрока, Строки);
...
Функция DOCTYP_Банк(Знач DOCTYP)
	Возврат БанкСписание(DOCTYP) Или БанкПоступление(DOCTYP) Или БанкПлюсМинус(DOCTYP);
КонецФункции

Функция БанкСписание(Знач DOCTYP)
	Возврат (СтрНайти("BD BL BO BS BV TR TS BZ", ВРег(DOCTYP)) > 0);
КонецФункции

Функция БанкПоступление(Знач DOCTYP)
	Возврат (СтрНайти("BM DA KA", ВРег(DOCTYP)) > 0);
КонецФункции

Функция БанкПлюсМинус(Знач DOCTYP)
	Возврат (СтрНайти("BU", ВРег(DOCTYP)) > 0);
КонецФункции

Функция БанкКонвертация(Знач DOCTYP)
	Возврат (СтрСравнить("BJ", DOCTYP) = 0);
КонецФункции
...
Процедура ПринятиеКУчетуОССоздать(Шапка, Детали)
	...
	ДокЗаписать(Док, Ошибки, Шапка.DOCNO);
КонецПроцедуры
...
Функция ТакиСписание(Детали)
	Ответ = Ложь;
	Для Каждого Деталь Из Детали Цикл
		Если Лев(Деталь.ALT_ACCNO, 2) = "51" Тогда
			Ответ = (Деталь.AMOUNT_COMPANYCODECURR < 0);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Возврат Ответ;
...
Процедура ВыпискаБанкаСоздать(Шапка, Детали)
	Ошибки			= Новый Массив;
	БанковскийСчет	= БанковскийСчетОрганизацииНайти(Шапка, СчетаУчета(Детали, "51"));
	Если НЕ ЗначениеЗаполнено(БанковскийСчет) Тогда
		БанковскийСчет	= БанковскийСчетОрганизацииНайти(Шапка, СчетаУчета(Детали, "52"));
		Если НЕ ЗначениеЗаполнено(БанковскийСчет) Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		КонецЦикла;
		Если НЕ ПустаяСтрока(Шапка.REFDOCNO_TOREVERSE) Тогда
			Если ШалостьУдалась Тогда
				ДокЗаписать(Док,, Шапка.DOCNO, Истина);
			КонецЕсли;
			ZFI_011_RFC_OZBEKISTAN_RTRN(, Шапка.DOCNO);
			Возврат;
		ИначеЕсли Док.Проведен Тогда
			Если НЕ ДокОтменить(Док) Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;

	ИначеЕсли ПустаяСтрока(Шапка.REFDOCNO_TOREVERSE) Тогда
	...
	ALT_ACCNO			= БанковскийСчет.СчетУчета.Код;
	...
	Для Каждого Деталь Из Детали Цикл
		Если НЕ ПустаяСтрока(Деталь.TAXNO) И Деталь.TAXNO <> AsfarmaКод() Тогда
			ДопДанные.Субконто1		= КонтрагентНайти(Деталь.TAXNO);
			ДопДанные.Субконто2		= ДоговорКонтрагентаПолучить(ДопДанные.Субконто1, ВалютаПолучить(Деталь.CURRENCY), Деталь.CONTRACT_NO);
			ДопДанные.СтатьяДвиженияДенежныхСредств	= СтатьяДвиженияДенежныхСредствПодобрать(ДопДанные, БанковскийСчет);
		КонецЕсли;
		Если БанкСписание(Деталь.DOCTYP) Или (БанкПлюсМинус(Деталь.DOCTYP) И ТакиСписание(Детали)) Тогда
			Если Деталь.ALT_ACCNO <> ALT_ACCNO Тогда
				ТекСтрока = Док.Платежи.Добавить();
				ТекСтрока.ПлатежныйДокумент	= Деталь.DOCNO;
				ТекСтрока.Валюта			= ВалютаПолучить(Деталь.CURRENCY);
				ТекСтрока.Курс				= Абс(Деталь.EXCRATE * 1000);
				...
				ТекСтрока.Расход			= Абс(Деталь.AMOUNT_COMPANYCODECURR);
				Если Док.СчетБанк.Валютный Тогда
					ТекСтрока.РасходВВалюте		= Абс(Деталь.AMOUNT_TRANSACTIONCURR);
				КонецЕсли;
				Если ТекСтрока.КоррСчет.Валютный Тогда
					ТекСтрока.РасходВКорВалюте	= Абс(Деталь.AMOUNT_TRANSACTIONCURR);
				КонецЕсли;
				
				ЗаполнитьЗначенияСвойств(ТекСтрока, ДопДанные);
				ПлатежДополнить(ТекСтрока, БанковскийСчет);
				Прервать;
			КонецЕсли;
			
		ИначеЕсли БанкПоступление(Деталь.DOCTYP) Или БанкПлюсМинус(Деталь.DOCTYP) Тогда
			Если Деталь.ALT_ACCNO <> ALT_ACCNO Тогда
				ТекСтрока = Док.Платежи.Добавить();
				ТекСтрока.ПлатежныйДокумент	= Деталь.DOCNO;
				ТекСтрока.Валюта			= ВалютаПолучить(Деталь.CURRENCY);
				...
				ТекСтрока.Приход			= Абс(Деталь.AMOUNT_COMPANYCODECURR);
				Если Док.СчетБанк.Валютный Тогда
					ТекСтрока.ПриходВВалюте		= Абс(Деталь.AMOUNT_TRANSACTIONCURR);
				КонецЕсли;
				Если ТекСтрока.КоррСчет.Валютный Тогда
					ТекСтрока.ПриходВКорВалюте	= Абс(Деталь.AMOUNT_TRANSACTIONCURR);
				КонецЕсли;
...
Процедура КонвертацияБанкСоздать(Шапка, Детали)
	Ошибки			= Новый Массив;
	БанковскийСчет	= БанковскийСчетОрганизацииНайти(Шапка, СчетаУчета(Детали, Макс("55", Шапка.ALT_ACCNO)));
	Для Каждого Деталь Из Детали Цикл
		Если ЗначениеЗаполнено(БанковскийСчет) Тогда
			Прервать;
		ИначеЕсли НЕ СчетУчетаБанк(Деталь.ALT_ACCNO) Тогда
			БанковскийСчет	= БанковскийСчетОрганизацииНайти(Деталь, СчетаУчета(Детали, Деталь.ALT_ACCNO));
		КонецЕсли;
	КонецЦикла;
	Если НЕ ЗначениеЗаполнено(БанковскийСчет) Тогда
		Возврат;
	...
	Для Каждого Деталь Из Детали Цикл
		Если БанкКонвертация(Деталь.DOCTYP) Тогда
			Если СчетУчетаБанк(Деталь.ALT_ACCNO) Тогда
				Если НЕ ЗначениеЗаполнено(ДопДанные.Субконто1) Тогда
					//ДопДанные.Субконто1		= БанковскийСчетНайтиПоНаименованию(Деталь.GLACC_TEXT, Деталь.ALT_ACCNO);
					//ДопДанные.Субконто2		= ДоговорКонтрагентаПолучить(ДопДанные.Субконто1, ВалютаПолучить(Деталь.CURRENCY), Деталь.CONTRACT_NO);
					ДопДанные.СтатьяДвиженияДенежныхСредств	= СтатьяДвиженияДенежныхСредствПодобрать(ДопДанные, БанковскийСчет);
				КонецЕсли;
				Если Деталь.AMOUNT_COMPANYCODECURR > 0 Тогда
					...
					ТекСтрока.ПлатежныйДокумент	= Деталь.DOCNO;
					ТекСтрока.Валюта			= ВалютаПолучить(Деталь.CURRENCY);
					ТекСтрока.Курс				= Деталь.EXCRATE * 1000;
					ТекСтрока.КоррСчет			= СчетУчетаНайти(Деталь.ALT_ACCNO);
					ТекСтрока.ДеталиПлатежа		= Деталь.COMMENTARY;

					ТекСтрока.ВидДвижения		= Перечисления.ВидДвиженияДенежныхСредствПоВыписке.Списание;
					Если ТекСтрока.Валюта <> НацВалюта() Тогда
...
Функция БанковскийСчетНайтиПоНаименованию(Наименование, СчетУчета)
	Ответ	= Справочники.БанковскиеСчета.ПустаяСсылка();
	Если ЗначениеЗаполнено(Наименование) Тогда
		Запрос	= Новый Запрос(
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	БанковскиеСчета.Ссылка КАК Ссылка,
		|	ЕСТЬNULL(КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВыпискаБанкаПлатежи.Ссылка), 0) КАК Количество
		|ИЗ
		...
		|			ПО ВыпискаБанкаПлатежи.Ссылка = ВыпискаБанка.Ссылка
		|		ПО БанковскиеСчета.Ссылка = ВыпискаБанкаПлатежи.Субконто1
		|ГДЕ
		|	БанковскиеСчета.Наименование = &Наименование
		|	И БанковскиеСчета.СчетУчета = &СчетУчета
		|	И ВыпискаБанка.Дата > &Дата
		|
		|СГРУППИРОВАТЬ ПО
...
Функция СчетУчетаБанк(Знач Код)
	Возврат (СтрНайти("51 52", Лев(СокрЛ(Код), 2)) > 0);
КонецФункции
...
Функция СтатьяДвиженияДенежныхСредствПодобрать(Параметры, БанковскийСчет)
	...
	Если ЗначениеЗаполнено(Параметры.Субконто2) Или ЗначениеЗаполнено(Параметры.Субконто1) Тогда
		...
		|ГДЕ
		|	Документ.БанковскийСчет = &БанковскийСчет
		|	И Документ.Дата > &Дата
		|	И Документ.Проведен = ИСТИНА
		|
		|СГРУППИРОВАТЬ ПО
...
Процедура ПлатежДополнить(Параметры, БанковскийСчет)
	Если НЕ ЗначениеЗаполнено(Параметры.СтатьяДвиженияДенежныхСредств) Тогда
		Запрос	= Новый Запрос(
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ВыпискаБанкаПлатежи.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств,
		|	ВыпискаБанкаПлатежи.Субконто1 КАК Субконто1,
		|	ВыпискаБанкаПлатежи.Субконто2 КАК Субконто2,
		|	ВыпискаБанкаПлатежи.Субконто3 КАК Субконто3
		|ИЗ
		|	Документ.ВыпискаБанка.Платежи КАК ВыпискаБанкаПлатежи
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВыпискаБанка КАК Документ
		|		ПО ВыпискаБанкаПлатежи.Ссылка = Документ.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СтатьиДвиженияДенежныхСредств КАК СтатьиДвиженияДенежныхСредств
		|		ПО ВыпискаБанкаПлатежи.СтатьяДвиженияДенежныхСредств = СтатьиДвиженияДенежныхСредств.Ссылка
		|ГДЕ
		|	Документ.БанковскийСчет = &БанковскийСчет
		|	И Документ.Дата > &Дата
		|	И Документ.Проведен = ИСТИНА
		|	И ВыпискаБанкаПлатежи.КоррСчет = &КоррСчет
		|	И ВыпискаБанкаПлатежи.Валюта = &Валюта
		|	И ВыпискаБанкаПлатежи.ВидДвижения = &ВидДвижения
		|
		|УПОРЯДОЧИТЬ ПО
		...
		//Запрос.УстановитьПараметр("Организация",	Организация);
		Запрос.УстановитьПараметр("БанковскийСчет",	БанковскийСчет);
		Запрос.УстановитьПараметр("КоррСчет",		Параметры.КоррСчет);
...
Процедура ОтветственныйУстановить(Док)
	Если ЕстьРеквизитДокумента("Ответственный", Док.Метаданные()) Тогда
		Док.Ответственный	= Ответственный();
	КонецЕсли;
КонецПроцедуры

Функция ДокОтменить(Док)
	Если КлючевоеСловоСодержится(Док.Комментарий) Тогда Возврат Ложь; КонецЕсли;
	ОтветственныйУстановить(Док);
	Попытка
		Док.Записать(РежимЗаписиДокумента.ОтменаПроведения);
	Исключение
		Логировать("Ошибка отмены проведения " + Строка(Док.Ссылка));
		Возврат Ложь;
	КонецПопытки;
	Возврат Истина;
КонецФункции

Процедура ДокЗаписать(Док, Ошибки=Неопределено, DOCNO, Принудительно=Ложь)
	Если НЕ Принудительно И КлючевоеСловоСодержится(Док.Комментарий) Тогда Возврат; КонецЕсли;
	ОтветственныйУстановить(Док);
	...
	Если Док.ПроверитьЗаполнение() Тогда
		Попытка
			...
			Логировать("Проведение " + Строка(Док.Ссылка) + " " + DOCNO, УровеньЖурналаРегистрации.Информация);
		Исключение
			//Если ТипЗнч(Ошибки) = Тип("Массив") Тогда
			//	Ошибки.Добавить(ОписаниеОшибки());
				//Док.Комментарий	= СтрСоединить(Ошибки, ",");
			Логировать("Ошибка проведения " + Строка(Док.Ссылка));
		КонецПопытки;
	//Иначе
	//	Логировать(ОписаниеОшибки());
	КонецЕсли;
	Если Док.Модифицированность() Тогда
		Попытка
			Док.Записать(РежимЗаписиДокумента.Запись);
		Исключение
			Логировать("Попытка записи " + Строка(Док.Ссылка));
		КонецПопытки;
	КонецЕсли;
КонецПроцедуры
...
Функция ZMM_000_FM_1C_FATURA_LIST_RTRN(ТЧ=Неопределено, SATINALMA_SIPARIS="") Экспорт
			...
			Если Загружено.Найти(ТекСтрока.SATINALMA_SIPARIS) = Неопределено Тогда
				Попытка
					Если ЗначениеЗаполнено(ПоступлениеНоменклатурыПродажВалютныйСпискамиНайти(ТекСтрока)) Тогда
						мСтроки = ТЧ.НайтиСтроки(Новый Структура("SATINALMA_SIPARIS", ТекСтрока.SATINALMA_SIPARIS));
						Для Каждого ТекСтрока2 Из мСтроки Цикл
							ЗаполнитьЗначенияСвойств(WSЗапись, ТекСтрока2);
							WSПараметр.item.Добавить(WSЗапись);
						КонецЦикла;
	...
	Если Загружено.Количество() > 0 И ПустаяСтрока(Ответ) Тогда
		...
		Логировать("ZMM_000_FM_1C_FATURA_LIST_RTRN " + ?(ПустаяСтрока(Ответ), СтрСоединить(Загружено, ";"), СтрСоединить(Загружено, ";") + Символы.ПС + Ответ), УровеньЖурналаРегистрации.Информация);
	КонецЕсли;
	Возврат Ответ;
КонецФункции
...
Функция ZSD_011_RFC_OZBFAT_IA_DKNT_RTR(ТЧ=Неопределено, BILLDOC="") Экспорт
	...
	Если ПустаяСтрока(BILLDOC) Тогда
		Для Каждого ТекСтрока Из ТЧ Цикл
			Если Загружено.Найти(ТекСтрока.BILLDOC) = Неопределено Тогда
				Попытка
					Если ?(SD_Отмена(ТекСтрока.BILLTYP), ЗначениеЗаполнено(SDДокументНайти(ТекСтрока)), SDДокументПроведен(ТекСтрока)) Тогда
						мСтроки = ТЧ.НайтиСтроки(Новый Структура("BILLDOC", ТекСтрока.BILLDOC));
						Для Каждого ТекСтрока2 Из мСтроки Цикл
							ЗаполнитьЗначенияСвойств(WSЗапись, ТекСтрока2);
							WSПараметр.item.Добавить(WSЗапись);
						КонецЦикла;
	...
	Если Загружено.Количество() > 0 И ПустаяСтрока(Ответ) Тогда
		...
		Логировать("ZSD_011_RFC_OZBFAT_IA_DKNT_RTR " + ?(ПустаяСтрока(Ответ), СтрСоединить(Загружено, ";"), СтрСоединить(Загружено, ";") + Символы.ПС + Ответ), УровеньЖурналаРегистрации.Информация);
	КонецЕсли;
	Возврат Ответ;
КонецФункции
...
Функция ЭтоПродуктив()
	Возврат (СтрНайти(НРег(СтрокаСоединенияИнформационнойБазы()), "asfarma82") > 0);
КонецФункции
