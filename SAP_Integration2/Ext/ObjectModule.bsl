...
Функция ZFI_011_RFC_OZBEKISTAN_RTRN(ТЧ=Неопределено, DOCNO="") Экспорт
	...
	Если Загружено.Количество() > 0 И ПустаяСтрока(Ответ) Тогда
		...
		Логировать("ZFI_011_RFC_OZBEKISTAN_RTRN " + ?(ПустаяСтрока(Ответ), ТекСтрока.DOCNO, Ответ), УровеньЖурналаРегистрации.Информация);
	КонецЕсли;
	Возврат Ответ;
КонецФункции
...
Процедура ВыпискаБанкаСоздать(Шапка, Детали)
	...
	Для Каждого Деталь Из Детали Цикл
		...
		КонецЕсли;
		Если БанкСписание(Деталь.DOCTYP) Тогда
		...
		ИначеЕсли БанкПоступление(Деталь.DOCTYP) Тогда
			Если Деталь.ALT_ACCNO <> ALT_ACCNO Тогда
				ТекСтрока = Док.Платежи.Добавить();
				ТекСтрока.ПлатежныйДокумент	= Деталь.DOCNO;
				ТекСтрока.Валюта			= ВалютаПолучить(Деталь.CURRENCY);
...
Процедура ДокЗаписать(Док, Ошибки=Неопределено)
	Если КлючевоеСловоСодержится(Док.Комментарий) Тогда Возврат; КонецЕсли;
	ОтветственныйУстановить(Док, Ответственный);
	...
	Если Док.ПроверитьЗаполнение() Тогда
		Попытка
			...
			Логировать("Проведение " + Строка(Док.Ссылка), УровеньЖурналаРегистрации.Информация);
		Исключение
			//Если ТипЗнч(Ошибки) = Тип("Массив") Тогда
...
Функция ZMM_000_FM_1C_FATURA_LIST_RTRN(ТЧ=Неопределено, SATINALMA_SIPARIS="") Экспорт
	...
	Если Загружено.Количество() > 0 И ПустаяСтрока(Ответ) Тогда
		...
		Логировать("ZMM_000_FM_1C_FATURA_LIST_RTRN " + ?(ПустаяСтрока(Ответ), ТекСтрока.SATINALMA_SIPARIS, Ответ), УровеньЖурналаРегистрации.Информация);
	КонецЕсли;
	Возврат Ответ;
КонецФункции
...
Функция ZSD_011_RFC_OZBFAT_IA_DKNT_RTR(ТЧ=Неопределено, BILLDOC="") Экспорт
	...
	Если Загружено.Количество() > 0 И ПустаяСтрока(Ответ) Тогда
		...
		Логировать("ZSD_011_RFC_OZBFAT_IA_DKNT_RTR " + ?(ПустаяСтрока(Ответ), ТекСтрока.BILLDOC, Ответ), УровеньЖурналаРегистрации.Информация);
	КонецЕсли;
	Возврат Ответ;
КонецФункции
