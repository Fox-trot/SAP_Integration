...
Функция ZFI_011_RFC_OZBEKISTAN_RTRN(ТЧ=Неопределено) Экспорт
	...
	Для Каждого ТекСтрока Из ТЧ Цикл
		Если Загружено.Найти(ТекСтрока.DOCNO) = Неопределено Тогда
			Попытка
				Если ЭтоТест Или УслугиСтороннихОрганизацийПроведен(ТекСтрока) Тогда
...
Функция ZMM_000_FM_1C_FATURA_LIST_RTRN(ТЧ=Неопределено) Экспорт
	Если ТЧ = Неопределено Тогда
		ТЧ = ZMM_000_S_1C_ENTEGRASYON_LIST;
	КонецЕсли;
	ТЧ.Сортировать("MUHASEBE_BELGE_NO,MUHASEBE_BELGE_KALEM");
	...
	Для Каждого ТекСтрока Из ТЧ Цикл
		Если Загружено.Найти(ТекСтрока.MUHASEBE_BELGE_NO) = Неопределено Тогда
			Попытка
				Если ЭтоТест Или ДокументРеализацииПроведен(ТекСтрока) Тогда
					ЗаполнитьЗначенияСвойств(WSЗапись, ТекСтрока);
					WSПараметр.item.Добавить(WSЗапись);
					Если ЭтоТест Тогда
						Прервать;
...
Функция ZSD_011_RFC_OZBFAT_IA_DKNT_RTR(ТЧ=Неопределено) Экспорт
	Если ТЧ = Неопределено Тогда
		ТЧ = ZSD_011_S_DATA;
	КонецЕсли;
	ТЧ.Сортировать("BILLDOC,ITEMNO");
