...
Процедура ZMM_000_FM_1C_FATURA_LISTESI(ТЧ=Неопределено) Экспорт
	...
	Попытка
		...
		Для Каждого ТекСтрока Из Объект.item Цикл
			...
			НовСтрока.MASRAF_TIPI_TX= Перевод(ТекСтрока.MASRAF_TIPI_TX);
		КонецЦикла;
	Исключение
		Сообщить(ОписаниеОшибки(), СтатусСообщения.ОченьВажное);
	КонецПопытки;
	...
	Для Каждого ТекСтрока Из ТЧ Цикл
		Если мДоки.Найти(ТекСтрока.MUHASEBE_BELGE_NO) = Неопределено Тогда
			Если ТекСтрока.KAYIT_TARIH < Дата(2021, 12, 1) Тогда
				//
			ИначеЕсли ДокументПоступленияСуществует(ТекСтрока) Тогда
...
Процедура ПоступлениеНоменклатурыВалютныйСоздать(Шапка, Детали)
	...
	Для Каждого Деталь Из Детали Цикл
		Если ПустаяСтрока(Деталь.MASRAF_TIPI) Тогда
			...
			//ТекСтрока.Номенклатура	= НоменклатураПродажПодобрать(Деталь.MALZEME, Деталь.PARTI, ?(ЭтоНацВалюта(Шапка.PARA_BIRIMI), Док.Поставщик.Наименование, "Импортные поставки " + Шапка.FATURA_NO), Деталь.SKT);
			ТекСтрока.Номенклатура	= НоменклатураПродажПодобрать(Деталь.MALZEME, Деталь.PARTI, "Импортные поставки " + Шапка.FATURA_NO, Деталь.SKT);
...
Функция НоменклатураПродажПодобрать(Знач Код, Знач Серия="", РодительНаименование="", СрокГодности=Неопределено)
	...
	Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Иначе
				...
				//РодительОбъект.ДатаРегистрации	= ;
			...
			Справочник.СрокГодности	= СрокГодности;
