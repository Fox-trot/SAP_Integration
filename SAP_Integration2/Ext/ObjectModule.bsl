...
Процедура ZMM_000_FM_1C_FATURA_LISTESI(ТЧ=Неопределено) Экспорт
	...
	Попытка
		...
		Для Каждого ТекСтрока Из Объект.item Цикл
			...
			НовСтрока.MASRAF_TIPI_TX= ТекСтрока.MASRAF_TIPI_TX + " / " + Перевод(ТекСтрока.MASRAF_TIPI_TX);
		КонецЦикла;
	Исключение
		Сообщить(ОписаниеОшибки(), СтатусСообщения.ОченьВажное);
	КонецПопытки;
...
Процедура ПоступленияНоменклатурыСоздать(ТЧ=Неопределено) Экспорт
	Если ТЧ = Неопределено Тогда
		ТЧ = ZMM_000_S_1C_ENTEGRASYON_LIST;
	КонецЕсли;
	ТЧ.Сортировать("SATINALMA_SIPARIS,MUHASEBE_BELGE_NO,MUHASEBE_BELGE_KALEM");
	
	Настроить(ЭтотОбъект);
	мДоки	= Новый Массив;
	Для Каждого ТекСтрока Из ТЧ Цикл
		Если мДоки.Найти(ТекСтрока.SATINALMA_SIPARIS) = Неопределено Тогда
			Если ТекСтрока.KAYIT_TARIH < Дата(2021, 12, 1) Тогда
				//
			ИначеЕсли ДокументПоступленияПроведен(ТекСтрока) Тогда
...
Процедура ПоступлениеНоменклатурыВалютныйСоздать(Шапка, Детали)
	...
	Для Каждого Деталь Из Детали Цикл
		...
		Тогда
			СписокПоступления		= ?(Док.Номенклатура.Количество() = 0, Справочники.СпискиПоступленияНоменклатурыВВалюте.Список1, Док.Номенклатура.Получить(0).Список);
			...
			КонецЕсли;
		Иначе
			мСтроки	= Док.РасходыВключаемыеВСтоимость.НайтиСтроки(Новый Структура("Описание,Валюта",  Деталь.MASRAF_TIPI_TX, ВалютаПолучить(Деталь.PARA_BIRIMI)));
			Если мСтроки.Количество() = 0 Тогда
				...
				ТекСтрока.Субконто1		= КонтрагентПолучить(Деталь.SATICI, Деталь.SATICI_TX);
...
Функция Таможня()
	Если НЕ ЗначениеЗаполнено(Таможня) Тогда
		Таможня	= КонтрагентПолучить("200637640");
	КонецЕсли;
	Возврат Таможня;
КонецФункции
...
Функция КонтрагентПолучить(ИНН, Наименование="~")
...
Функция НоменклатураПродажПодобрать(Знач Код, Знач Серия="", Знач РодительНаименование="", СрокГодности=Неопределено)
	...
	Тогда
		...
		Пока СтрДлина(Код) > 5 Цикл
			...
			Коды.Добавить(Код);
		КонецЦикла;
		Запрос.УстановитьПараметр("Код",	Коды);
		//Запрос.УстановитьПараметр("Серия",	Серия);
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
...
Процедура РеализацияМедикаментовСоздатьНаСервере(ТЧ=Неопределено) Экспорт
	...
	Для Каждого ТекСтрока Из ТЧ Цикл
		Если мДоки.Найти(ТекСтрока.BILLDOC) = Неопределено Тогда
			Если ТекСтрока.BILLDATE < Дата(2021, 12, 1) Тогда
				//
			ИначеЕсли ДокументПроведен(ТекСтрока) Тогда
				
			Иначе
				Строки = ТЧ.НайтиСтроки(Новый Структура("BILLDOC", ТекСтрока.BILLDOC));
				Если СтрСравнить(ТекСтрока.BILLTYP, "ZI02") = 0 Тогда
...
Процедура РеализацияМедикаментовСоздать(Шапка, Детали)
	...
	Для Каждого Деталь Из Детали Цикл
		...
		//Иначе	//Если СтрСравнить(Деталь.BILLTYP, "ZS07") = 0 Тогда
			...
			ТекСтрока.Себестоимость	= Окр(Остаток.Сумма / Остаток.Количество, 2);
			ТекСтрока.Сумма			= Деталь.TOTALPRICE;
			ТекСтрока.Наценка		= Деталь.PROFITMARJIN;
			ТекСтрока.СтавкаНДС		= ТекСтрока.Номенклатура.СтавкаНДС;
			ТекСтрока.СуммаНДС		= Деталь.KDV;
...
Процедура ВозвратНоменклатурыПродажСоздать(Шапка, Детали)
	...
	Если НЕ ЗначениеЗаполнено(Док.Склад) Тогда
		Док.Склад			= СкладПолучить();
	КонецЕсли;
	//Док.КорректируемыйДокументРеализации
	Если НЕ ЗначениеЗаполнено(Док.Контрагент) Тогда
		Док.Контрагент		= КонтрагентПолучить(Шапка.VKN);
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Док.Контрагент) Тогда
...
Функция ДокументПроведен(Параметры)
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	Документ.Ссылка КАК Ссылка
	|ИЗ
...
Процедура УслугиСтороннихСоздатьНаСервере(ТЧ=Неопределено) Экспорт
	...
	Для Каждого ТекСтрока Из ТЧ Цикл
		Если мДоки.Найти(ТекСтрока.DOCNO) = Неопределено Тогда
			Если ТекСтрока.DOCDATE < Дата(2021, 12, 1) Тогда
				//
			//ИначеЕсли СтрСравнить(ТекСтрока.DOCTYP_DESC, "IFT dosya kapama") = 0 Тогда
			...
			ИначеЕсли СтрСравнить(Лев(ТекСтрока.DOCTYP_DESC, 3), "IFT") = 0 Тогда
				//
			ИначеЕсли УслугиСтороннихОрганизацийСуществует(ТекСтрока) Тогда
...
Процедура УслугиСтороннихОрганизацийСоздать(Шапка, Детали)
	...
	Для Каждого Деталь Из Детали Цикл
		...
		//ТекСтрока.СтавкаНДС			= 
		//ТекСтрока.СуммаНДС			= 
		ТекСтрока.СчетЗатрат		= Док.СчетЗатрат;
		//ТекСтрока.Субконто1			= ;
		//ТекСтрока.Субконто2			= ;
	КонецЦикла;
...
Функция Перевод(Фраза)
	Если Переводы.Количество() = 0 Тогда
		...
		Для Итератор = 1 По Макет.ВысотаТаблицы Цикл
			...
			ТекСтрока.Фраза		= Макет.Область(Итератор, 1, Итератор, 1).Текст;
...
Процедура ZMM_000_FM_1C_FATURA_LIST_RTRN(ТЧ=Неопределено) Экспорт
	Если ТЧ = Неопределено Тогда
		ТЧ = ZMM_000_S_1C_ENTEGRASYON_LIST;
	КонецЕсли;
	...
	Для Каждого ТекСтрока Из ТЧ Цикл
		Если мДоки.Найти(ТекСтрока.SATINALMA_SIPARIS) = Неопределено Тогда
			Если ТекСтрока.KAYIT_TARIH < Дата(2021, 12, 1) Тогда
				//
			ИначеЕсли ДокументПоступленияПроведен(ТекСтрока) Тогда
				WSПараметр	= WSПрокси.ФабрикаXDTO.Создать(WSПрокси.ФабрикаXDTO.Тип(URIПространстваИмен, "ZMM_000_S_1C_FATURA_LIST_LOG"));
				WSПараметр.SIRKET_KODU			= ТекСтрока.SIRKET_KODU;
				WSПараметр.YIL					= ТекСтрока.YIL;
				WSПараметр.MUHASEBE_BELGE_NO	= ТекСтрока.MUHASEBE_BELGE_NO;
				WSПараметр.MUHASEBE_BELGE_KALEM	= ТекСтрока.MUHASEBE_BELGE_KALEM;
				WSПараметр.FATURA_NO			= ТекСтрока.FATURA_NO;
				ОбъектХ	= WSПрокси.ZMM_000_FM_1C_FATURA_LIST_RTRN(WSПараметр);
				Если НЕ ПустаяСтрока(ОбъектХ.MSGV1) Тогда
					//ОбъектХ.MSGTY
					Сообщить(ОбъектХ.MSGV1);
				КонецЕсли;
			КонецЕсли;
