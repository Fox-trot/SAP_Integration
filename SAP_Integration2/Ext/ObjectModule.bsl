...
Процедура ПринятиеКУчетуОССоздать(Шапка, Детали)
	...
	Для Каждого Деталь Из Детали Цикл
		...
		Если Деталь.AMOUNT_COMPANYCODECURR > 0 Тогда
			ТекСтрока.ОсновноеСредство	= ОсновноеСредствоНайти(Деталь.ALT_ACCNO);
			ТекСтрока.ИнвентарныйНомер	= Деталь.ALT_ACCNO;
...
Процедура ВыпискаБанкаСоздать(Шапка, Детали)
	Ошибки		= Новый Массив;
	Ссылка		= ВыпискаБанкаНайти(Шапка);
	Если ЗначениеЗаполнено(Ссылка) Тогда
		Док = Ссылка.ПолучитьОбъект();
		Если Док.ПометкаУдаления Тогда
			Док.УстановитьПометкуУдаления(Ложь);
	...
	КонецЕсли;
	Док.БанковскийСчет	= БанковскийСчетОрганизацииНайти(Шапка, Док.ВалютаДокумента);
	//Док.СчетБанк			= Шапка.;
	...
	Для Каждого Деталь Из Детали Цикл
		Если Деталь.AMOUNT_COMPANYCODECURR > 0 Тогда
			...
			ТекСтрока.ДеталиПлатежа		= Деталь.GLACC_TEXT;
			//ТекСтрока.СтатьяДвиженияДенежныхСредств	= ;
		...
		Иначе
			Если ТекСтрока = Неопределено Тогда
				ТекСтрока = Док.Платежи.Добавить();
			КонецЕсли;
			Если НЕ ПустаяСтрока(Шапка.TAXNO) И НЕ ЗначениеЗаполнено(ТекСтрока.Субконто1) Тогда
				ТекСтрока.Субконто1		= КонтрагентНайти(Шапка.TAXNO);
				Если НЕ ЗначениеЗаполнено(ТекСтрока.Субконто1) Тогда
					Ошибки.Добавить(СтрШаблон("Не найден Контрагент по коду %1", Шапка.TAXNO));
				ИначеЕсли НЕ ЗначениеЗаполнено(ТекСтрока.Субконто2) Тогда
					ТекСтрока.Субконто2		= ДоговорКонтрагентаПолучить(ТекСтрока.Субконто1);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция БанковскийСчетОрганизацииНайти(Параметры, Валюта)
	Ответ	= Справочники.БанковскиеСчета.ПустаяСсылка();
	Запрос	= Новый Запрос(
	"ВЫБРАТЬ
	|	БанковскиеСчета.Ссылка КАК Ссылка,
	|	ЕСТЬNULL(ВыпискаБанка.Дата, &Дата) КАК Дата
	|ИЗ
	|	Справочник.БанковскиеСчета КАК БанковскиеСчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВыпискаБанка КАК ВыпискаБанка
	|		ПО БанковскиеСчета.Ссылка = ВыпискаБанка.БанковскийСчет
	|			И (ВыпискаБанка.Проведен = ИСТИНА)
	|ГДЕ
...
Функция ВыпискаБанкаНайти(Параметры)
	Ответ	= Документы.ПринятиеКУчетуОС.ПустаяСсылка();
	Запрос	= Новый Запрос(
	"ВЫБРАТЬ
	|	Документ.Ссылка КАК Ссылка
