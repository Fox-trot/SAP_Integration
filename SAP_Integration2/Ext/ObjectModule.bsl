...
Функция ZFI_011_RFC_OZBEKISTAN_RTRN(ТЧ=Неопределено) Экспорт
	Ответ	= "";
	...
	Шерлок		= Новый Структура("DOCNO");
	Для Каждого ТекСтрока Из ТЧ Цикл
		ЗаполнитьЗначенияСвойств(Шерлок, ТекСтрока.DOCNO);
		Если Загружено.Найти(Шерлок) = Неопределено Тогда
			Попытка
				ЗаполнитьЗначенияСвойств(WSПараметр.Добавить(), ТекСтрока);
...
Процедура ПоступлениеНоменклатурыВалютныйСоздать(Шапка, Детали)
	...
	Для Каждого Деталь Из Детали Цикл
		...
		Тогда
			...
			КонецЕсли;
		Иначе
			Описание= Перевод(ТекСтрока.MASRAF_TIPI_TX);
