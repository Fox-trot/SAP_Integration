...
Процедура ZFI_011_RFC_OZBEKISTAN(ТЧ=Неопределено) Экспорт
	...
	Попытка
		...
		Для Каждого ТекСтрока Из Объект.DATA.item Цикл
			//Если СтрокаКакДата(ТекСтрока.DOCDATE) >= Дата(2021, 12, 1) Тогда
...
Процедура ПринятиеКУчетуОС_Дефолт(Док)
	...
	|	Документ.Дата УБЫВ");
	Запрос.УстановитьПараметр("Организация",	Организация);
	Запрос.УстановитьПараметр("Дата",			Дата(2021, 12, 1));
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
...
Процедура СчетФактураВыданныйСоздать(Шапка, Детали)
	...
	Для Каждого Деталь Из Детали Цикл
		Если СтрСравнить(Деталь.ALT_ACCNO, "4415") Тогда //	 - ндс
			//Док.СтавкаНДС			= ;
