...
Функция ПечатьНакладная()
	...
	Для Каждого СтрокаНоменклатуры из Материалы Цикл
		...
		Область.Параметры.Цена = ?(СтрокаНоменклатуры.Количество<>0,Окр(СтрокаНоменклатуры.Сумма/СтрокаНоменклатуры.Количество,2),0);
...
Процедура ДвиженияПоБУПоУказаннойСтоимости(Режим, Отказ, Заголовок ,СтруктураШапкиДокумента)
	...
	Для Каждого СтрокаНоменклатуры Из Материалы Цикл
		...
		УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,1,СтрокаНоменклатуры.НоменклатураДт);
		УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,2,СкладПолучатель);
		Проводка.КоличествоДт = СтрокаНоменклатуры.КоличествоДт;
...
Процедура ПроверитьЗаполнениеДокумента(Отказ, Заголовок, СтруктураШапкиДокумента)
	...
	СтруктураПолей.Вставить("НоменклатураДт","Не выбрана номенклатура Дт");
	СтруктураПолей.Вставить("Номенклатура","Не выбрана номенклатура Кт");
