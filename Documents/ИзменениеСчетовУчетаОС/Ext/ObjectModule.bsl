...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь, НепосредственнаяПечать = Ложь) Экспорт
	...
	//	Предупреждение("Для печати запишите документ!");
	//	Возврат;
...
Процедура ДвиженияПоРегистрамБУ(СтруктураШапкиДокумента, ТаблицаПоОС,Отказ, Заголовок)
	...
	//	
		...
		УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,2,СтрокаТЧ.Подразделение);
		УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,3,СтрокаТЧ.МОЛ);
		...
		УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,2,СтрокаТЧ.Подразделение);
		УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,3,СтрокаТЧ.МОЛ);
		УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,4,СтрокаТЧ.ГруппаОСОткуда);
		Проводка.Сумма=СтрокаТЧ.СтоимостьБУ;
		
		Если (СтрокаТЧ.АмортизацияБУ)<>0 Тогда
			...
			Проводка.Сумма=СтрокаТЧ.АмортизацияБУ;
...
//	ПроверитьДублиОС(ТаблицаПоОС, Отказ, Заголовок);
	...
	СписокОС =
	"ВЫБРАТЬ
	|	ИзменениеСчетовУчетаОСОС.ОсновноеСредство КАК ОсновноеСредство
	|ИЗ
	|	Документ.ИзменениеСчетовУчетаОС.ОС КАК ИзменениеСчетовУчетаОСОС
	|ГДЕ
	|	ИзменениеСчетовУчетаОСОС.Ссылка = &Ссылка";
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ИзменениеСчетовУчетаОСОС.НомерСтроки,
	|	ИзменениеСчетовУчетаОСОС.ОсновноеСредство,
	|	ИзменениеСчетовУчетаОСОС.СтоимостьБУ,
	|	ИзменениеСчетовУчетаОСОС.АмортизацияБУ,
	|	ИзменениеСчетовУчетаОСОС.СчетНачисленияАмортизацииКуда,
	|	ИзменениеСчетовУчетаОСОС.СчетУчетКуда,
	|	ИзменениеСчетовУчетаОСОС.ГруппаОСКуда,
	|	ИзменениеСчетовУчетаОСОС.ГруппаОСОткуда,
	|	ИзменениеСчетовУчетаОСОС.Подразделение,
	|	ИзменениеСчетовУчетаОСОС.МОЛ,
	...
	|		ПО ИзменениеСчетовУчетаОСОС.ОсновноеСредство = ПервоначальныеСведенияОСБухгалтерскийУчетСрезПоследних.ОсновноеСредство
	...
	|		ПО ИзменениеСчетовУчетаОСОС.ОсновноеСредство = СчетаБухгалтерскогоУчетаОССрезПоследних.ОсновноеСредство
	...
	|		ПО ИзменениеСчетовУчетаОСОС.ОсновноеСредство = СпособыОтраженияРасходовПоАмортизацииОСБухгалтерскийУчетСрезПоследних.ОсновноеСредство
	...
	|		ПО ИзменениеСчетовУчетаОСОС.ОсновноеСредство = ОС_СнятыеСУчетаБУ.ОсновноеСредство
	|ГДЕ
	|	ИзменениеСчетовУчетаОСОС.Ссылка = &Ссылка";
