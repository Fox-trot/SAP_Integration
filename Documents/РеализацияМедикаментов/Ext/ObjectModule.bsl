...
Функция ПечатьСчетФактурыРазрешено()
	...
	Если Дата >= Дата(2022, 1, 1) Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	ВЫБОР
		               |		КОГДА (ВЫРАЗИТЬ(Товары.СуммаНДС КАК ЧИСЛО(20, 0))) = 0
		               |			ТОГДА 0
		               |		ИНАЧЕ Товары.Количество * (ВЫРАЗИТЬ(Товары.Цена КАК ЧИСЛО(20, 0))) + (ВЫРАЗИТЬ(Товары.СуммаНДС КАК ЧИСЛО(20, 0)))
		               |	КОНЕЦ КАК Всего,
		               ...
		               |	РеализацияМедикаментовУслуги.СтавкаНДС,
		               |	ВЫБОР
		               |		КОГДА (ВЫРАЗИТЬ(РеализацияМедикаментовУслуги.СуммаНДС КАК ЧИСЛО(20, 0))) = 0
		               |			ТОГДА 0
		               |		ИНАЧЕ (ВЫРАЗИТЬ(РеализацияМедикаментовУслуги.Сумма КАК ЧИСЛО(20, 0))) + (ВЫРАЗИТЬ(РеализацияМедикаментовУслуги.СуммаНДС КАК ЧИСЛО(20, 0)))
		               |	КОНЕЦ,
