...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если СуммаПоДокументу <> Сумма Тогда СуммаПоДокументу = Сумма; КонецЕсли;
КонецПроцедуры
...
Функция ПечатьСчетФактура()
	...
	|ПО
	|	ОБЩИЕ";
	Если Дата >= Дата(2022, 1, 1) Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	Товары.НомерСтроки КАК НомерСтроки,
		               |	Товары.Номенклатура КАК Номенклатура,
		               |	Товары.СрокГодности КАК СрокГодности,
		               |	Товары.Количество КАК Количество,
		               |	Товары.Количество * (ВЫРАЗИТЬ(Товары.Цена КАК ЧИСЛО(20, 0))) КАК Сумма,
		               |	Товары.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		               |	ВЫРАЗИТЬ(Товары.Цена КАК ЧИСЛО(20, 0)) КАК Цена,
		               |	ВЫРАЗИТЬ(Товары.СуммаНДС КАК ЧИСЛО(20, 0)) КАК СуммаНДС,
		               |	Товары.СтавкаНДС КАК СтавкаНДС,
		               |	Товары.Количество * (ВЫРАЗИТЬ(Товары.Цена КАК ЧИСЛО(20, 0))) + (ВЫРАЗИТЬ(Товары.СуммаНДС КАК ЧИСЛО(20, 0))) КАК Всего,
		               |	Товары.Количество * (ВЫРАЗИТЬ(Товары.Цена КАК ЧИСЛО(20, 0))) + (ВЫРАЗИТЬ(Товары.СуммаНДС КАК ЧИСЛО(20, 0))) КАК СуммаСУчетомАкцизаИНДС,
		               |	Товары.Серия КАК Серия,
		               |	Товары.Себестоимость КАК Себестоимость,
		               |	Товары.Наценка КАК Наценка,
		               |	Товары.Номенклатура.ПроисхождениеТовара КАК Происхождение
		               |ИЗ
		               |	Документ.РеализацияМедикаментов.Номенклатура КАК Товары
		               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияМедикаментов КАК РеализацияМедикаментов
		               |		ПО Товары.Ссылка = РеализацияМедикаментов.Ссылка
		               |ГДЕ
		               |	РеализацияМедикаментов.Ссылка = &Ссылка
		               |
		               |ОБЪЕДИНИТЬ ВСЕ
		               ...
		               |	NULL,
		               |	1,
		               |	ВЫРАЗИТЬ(РеализацияМедикаментовУслуги.Сумма КАК ЧИСЛО(20, 0)),
		               |	РеализацияМедикаментовУслуги.Услуга.БазоваяЕдиницаИзмерения,
		               |	ВЫРАЗИТЬ(РеализацияМедикаментовУслуги.Сумма КАК ЧИСЛО(20, 0)),
		               |	ВЫРАЗИТЬ(РеализацияМедикаментовУслуги.СуммаНДС КАК ЧИСЛО(20, 0)),
		               |	РеализацияМедикаментовУслуги.СтавкаНДС,
		               |	(ВЫРАЗИТЬ(РеализацияМедикаментовУслуги.Сумма КАК ЧИСЛО(20, 0))) + (ВЫРАЗИТЬ(РеализацияМедикаментовУслуги.СуммаНДС КАК ЧИСЛО(20, 0))),
		               |	(ВЫРАЗИТЬ(РеализацияМедикаментовУслуги.Сумма КАК ЧИСЛО(20, 0))) + (ВЫРАЗИТЬ(РеализацияМедикаментовУслуги.СуммаНДС КАК ЧИСЛО(20, 0))),
		               |	"""",
		               |	0,
		               |	0,
	...
	Если ПечататьАкт Тогда
		...
		Всего = ?(Дата >= Дата(2022, 1, 1), Окр(ВыборкаОбщие.Сумма, 0) + Окр(ВыборкаОбщие.СуммаНДС, 0), ВыборкаОбщие.Сумма + ВыборкаОбщие.СуммаНДС);
	...
	Если ФинансоваяСкидкаПроцент=0  И флФинСкидка = ЛОжь Тогда
		Область.Параметры.ВсегоПрописью	= СформироватьСуммуПрописью(Всего, Константы.ВалютаРегламентированногоУчета.Получить());
	ИначеЕсли Дата >= Дата(2022, 1, 1) Тогда
		Область.Параметры.ФинансоваяСкидкаПроцент	= ФинансоваяСкидкаПроцент;
		Область.Параметры.ФинансоваяСкидкаСумма		= Окр(ФинансоваяСкидкаСумма, 0) + ?(флФинСкидка, Окр(ФинСкидка2, 0), 0);
		Область.Параметры.СуммаБезСкидки			= Всего - Окр(ФинансоваяСкидкаСумма, 0) - ?(флФинСкидка, Окр(ФинСкидка2, 0), 0);
		Область.Параметры.ВсегоПрописью				= СформироватьСуммуПрописью(Всего - Окр(ФинансоваяСкидкаСумма, 0) - ?(флФинСкидка, Окр(ФинСкидка2, 0), 0), Константы.ВалютаРегламентированногоУчета.Получить());
	Иначе
		Область.Параметры.ФинансоваяСкидкаПроцент=ФинансоваяСкидкаПроцент;
...
Функция СуммаПоДокументу()
	Возврат Номенклатура.Итог("Сумма") + Номенклатура.Итог("СуммаНДС") + Услуги.Итог("Сумма") + Услуги.Итог("СуммаНДС");
КонецФункции
