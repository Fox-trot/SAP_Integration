...
Функция ПечатьПП(ИмяМакета)
	...
	Область.Параметры.СуммаЭквивалентПрописью=ПроцедурыСервера.СформироватьСуммуПрописью_Сервер(СуммаЭквивалент,Константы.ВалютаРегламентированногоУчета.Получить());
	Область.Параметры.СуммаВыручкиПрописью=ПроцедурыСервера.СформироватьСуммуПрописью_Сервер(СуммаВыручки,Валюта);
	Область.Параметры.СуммаЗачисленияПрописью=ПроцедурыСервера.СформироватьСуммуПрописью_Сервер(СуммаЗачисления,Валюта);
	Область.Параметры.СуммаПродажиПрописью=ПроцедурыСервера.СформироватьСуммуПрописью_Сервер(СуммаПродажи,Валюта);
