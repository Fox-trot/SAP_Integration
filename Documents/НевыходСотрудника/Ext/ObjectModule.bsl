...
Процедура ДвиженияПоРегистрам(Режим, Отказ, Заголовок,СтруктураШапкиДокумента)
	...
	Если Списком Тогда
		Для Каждого Строка Из Сотрудники Цикл
			ДвиженияПоСотруднику(Отказ,Строка.Сотрудник,Строка.НачалоПериода,Строка.КонецПериода);
		КонецЦикла;
	Иначе
		ДвиженияПоСотруднику(Отказ,Сотрудник,НачалоПериода,КонецПериода);
	КонецЕсли;
	
КонецПроцедуры // Печать()

Процедура ДвиженияПоСотруднику(Отказ,Сотрудник,НачалоПериода,КонецПериода)
	
	СведенияОСотруднике=СведенияОСотруднике(Сотрудник,Организация,НачалоПериода);

	Если не ЗначениеЗаполнено(СведенияОСотруднике) Тогда
		Сообщить("Не найдены сведений о сотруднике "+Строка(Сотрудник),СтатусСообщения.Важное);
		Отказ=Истина;
		Возврат;
	КонецЕсли;	
