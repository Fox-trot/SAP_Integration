...
#Если Клиент Тогда
	...
	Функция ПечатьСписаниеОС()
		...
		|	СписаниеОСОС.ОсновноеСредство.ИнвентарныйНомер КАК ИнвНомер,
		|	ПервоначальныеСведенияОбОсновныхСредствахСрезПоследних.ПорядокПогашенияСтоимости КАК ПорядокПогашенияСтоимости,
		|	СписаниеОСОС.ПодразделениеБУ КАК Подразделение,
		|	СписаниеОСОС.МОЛБУ,
		|	СписаниеОСОС.ГруппаОС КАК Группа
		|ИЗ
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	ОбязательныеРеквизитыШапки = "Организация,СпособУчетаРасходовБУ";
...
Процедура ПроверкаРеквизитовТЧ(СтруктураШапкиДокумента, Отказ, Заголовок) Экспорт
	...
	РеквизитыТабНоменклатура = "Номенклатура,СкладОприходованияНоменклатуры"; //через запятую
	ПроверитьЗаполнениеТабличнойЧасти(ЭтотОбъект, "Номенклатура", Новый Структура(РеквизитыТабНоменклатура), Отказ, Заголовок);
...
Процедура ДвиженияПоРегистрамРегл(СтруктураШапкиДокумента, ТаблицаПоОС, Отказ, Заголовок)
	...
	Для Каждого СтрокаТЧ Из ТаблицаПоОС Цикл
		...
		Если СтрокаТЧ.СтоимостьБУ <> 0 Тогда
			...
			Проводка.СчетДт = ПланыСчетов.Хозрасчетный.А9210;
		...
		Если СуммаПроводки <> 0 Тогда
			...
			Проводка.СчетКт = ПланыСчетов.Хозрасчетный.А9210;
		...
		ТекСуммаПрибылиУбытка = СтрокаТЧ.СтоимостьБУ - СтрокаТЧ.АмортизацияБУ - СтрокаТЧ.АмортизацияЗаМесяцБУ- СтрокаТЧ.ПереоценкаБУ+СтрокаТЧ.ВозвратАмортизацииБУ;
		
		СуммаПрибылиУбытка = СуммаПрибылиУбытка+ТекСуммаПрибылиУбытка;
		
		Если ПриСписанииОСОтражатьПрибыльУбытокПоКаждомуОСОтдельно Тогда
			Если ТекСуммаПрибылиУбытка > 0 Тогда
				
				Проводка = ПроводкиБУ.Добавить();
				
				Проводка.Период       = Дата;
				Проводка.Организация  = ТекОрганизация;
				Проводка.Содержание   = "Убытки от списания ОС";
				Проводка.НомерЖурнала = НомерЖурнала;
				Проводка.Сумма        = ТекСуммаПрибылиУбытка;
			...
			ИначеЕсли ТекСуммаПрибылиУбытка < 0 Тогда	
				Проводка = ПроводкиБУ.Добавить();
				
				Проводка.Период       = Дата;
				Проводка.Организация  = ТекОрганизация;
				Проводка.Содержание   = "Прибыль от списания ОС";
				
				Проводка.НомерЖурнала = НомерЖурнала;
				Проводка.Сумма        = -ТекСуммаПрибылиУбытка;
				
				Проводка.СчетДт = ПланыСчетов.Хозрасчетный.А9210;
				//УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоКт, "ОсновныеСредства", ТекОС);
	...
	Если ПроводитьОприходованиеНоменклатуры Тогда
		...
		Для Каждого СтрокаНоменклатуры из Номенклатура Цикл
			...
			УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 2, СтрокаНоменклатуры.СкладОприходованияНоменклатуры);
	...
	Если Не ПриСписанииОСОтражатьПрибыльУбытокПоКаждомуОСОтдельно Тогда
		СуммаПрибылиУбытка = СуммаПрибылиУбытка-Номенклатура.Итог("Сумма");
		...
		ИначеЕсли СуммаПрибылиУбытка < 0 Тогда	
			Проводка = ПроводкиБУ.Добавить();
...
Процедура ОбработкаПроведения(Отказ)
	...
	|	СписаниеОСОС.ПереоценкаБУ,
	|	СписаниеОСОС.ВозвратАмортизацииБУ,
	|	СписаниеОСОС.МОЛБУ,
	|	СписаниеОСОС.ПодразделениеБУ,
	|	СписаниеОСОС.ГруппаОС,
	...
	|		ПО СписаниеОСОС.ОсновноеСредство = ОС_СнятыеСУчетаБУ.ОсновноеСредство
	|ГДЕ
	|	СписаниеОСОС.Ссылка = &Ссылка";
