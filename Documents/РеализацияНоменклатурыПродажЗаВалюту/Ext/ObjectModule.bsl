...
Функция ПечатьСчетФактура()
	...
	Область.Параметры.ВсегоПрописьюВВалюте	= ПроцедурыСервера.СформироватьСуммуПрописью_Сервер(Всего, Валюта);
	Область.Параметры.ВсегоПрописью			= ПроцедурыСервера.СформироватьСуммуПрописью_Сервер(ВсегоСум, Константы.ВалютаРегламентированногоУчета.Получить());
	Если Дата<Дата(2019,1,22) Тогда
