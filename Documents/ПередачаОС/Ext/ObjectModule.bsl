...
Функция ПечатьСчетФактура() Экспорт
	...
	Область.Параметры.ВсегоПрописью = ПроцедурыСервера.СформироватьСуммуПрописью_Сервер(ОС.Итог("Сумма") + ОС.Итог("СуммаНДС"), Константы.ВалютаРегламентированногоУчета.Получить());
	Если Дата<Дата(2019,1,22) Тогда
