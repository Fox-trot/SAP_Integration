...
Функция ПолучитьТаблицуМатериалов()
	...
	Иначе
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СписаниеНоменклатурыМатериалы.Сумма,
		|	СписаниеНоменклатурыМатериалы.Номенклатура,
		|	СписаниеНоменклатурыМатериалы.Номенклатура.Код КАК КодНоменклатуры,
		|	СписаниеНоменклатурыМатериалы.Номенклатура.БазоваяЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	СписаниеНоменклатурыМатериалы.Количество,
		|	СписаниеНоменклатурыМатериалы.ПричинаСписания,
		|	ВЫБОР
		|		КОГДА СписаниеНоменклатурыМатериалы.Количество <> 0
		|			ТОГДА СписаниеНоменклатурыМатериалы.Сумма / СписаниеНоменклатурыМатериалы.Количество
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК Цена
		|ИЗ
		|	Документ.СписаниеНоменклатуры.Материалы КАК СписаниеНоменклатурыМатериалы
		|ГДЕ
		|	СписаниеНоменклатурыМатериалы.Ссылка = &Ссылка";
...
Функция ПечатьАктСписания()
	...
	Область.Параметры.КоличествоПрописью = ЧислоПрописью(ТМатериалы.Итог("Количество"),"НД=Ложь",",,,,,,,,0");
	...
	ТабДок.КлючПараметровПечати="КПП_СписаниеНоменклатуры";
...
Процедура ДвиженияПоБУПоУказаннойСтоимости(Режим, Отказ, Заголовок ,СтруктураШапкиДокумента)
	...
	Для Каждого СтрокаНоменклатуры Из Материалы Цикл
		...
		УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,2,?(ПроводитьЧерезТранзитныеСклады,СтрокаНоменклатуры.ТранзитныйСклад,Склад));
		Проводка.КоличествоКт = СтрокаНоменклатуры.Количество;
		Проводка.Сумма = СтрокаНоменклатуры.Сумма;
		
		Если ПроводитьЧерезТранзитныеСклады Тогда
			Проводка = ПроводкиБУ.Добавить();
			Проводка.Период      = Дата;
			Проводка.Организация = Организация;
			Проводка.Содержание = "";     
			...
			УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,2,СтрокаНоменклатуры.ТранзитныйСклад);
...
Процедура ДвиженияПоБУПоПартиям(Режим, Отказ, Заголовок, СтруктураШапкиДокумента)
	...
	Для Каждого СтрокаНоменклатуры Из ТМатериалы Цикл
		...
		Если СтрокаНоменклатуры.СтавкаНДС = Перечисления.СтавкиНДС.НДС20 И СписыватьНДС Тогда
			...
			КонецЕсли;
			Проводка.СчетКт     =  СчетУчетаПлатежа("НДС");
			Проводка.Сумма = СтрокаНоменклатуры.Сумма/5; 
